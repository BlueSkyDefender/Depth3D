...
{	
	...
	for (int j = 0; j <= 1; ++j) 
	{
		...
		color.rgb = tex2D(BackBuffer , float2(texcoord.xy-float2(SbSdepth(texcoord.xy)*Depth,0)*pix.xy)).rgb;

		colorT.rgb = tex2D(BackBuffer , float2(texcoord.xy+float2(SbSdepth(texcoord.xy)*Depth,0)*pix.xy)).rgb;
