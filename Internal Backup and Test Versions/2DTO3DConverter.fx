...
float Grade(float2 texcoord) 	
{

 float3 px = HQ4X(float2(texcoord.x,texcoord.y)).bbb;
 
 float3 py = 1 - texcoord.y+pix.y*tex2D(BackBuffer,float2(texcoord.x,texcoord.y)).rrr; 
...
{	
	const float samples[6] = {0.5, 0.66, 1, 0.25,0.75,0.01};
	...
	for (int j = 0; j <= 5; ++j) 
	{
		...
		DepthL=  min(DepthL,Grade(float2(texcoord.x+uv.x*pix.x, texcoord.y))).r;
		DepthR=  min(DepthR,Grade(float2(texcoord.x-uv.x*pix.x, texcoord.y))).r;
		
		//color.rgb = Grade(float2(texcoord.x, texcoord.y));
