...
static const float Anti_Weapon_Hand_ZFighting_D = 0.0;  //Anti-Weapon Hand Z-Fighting                   | AWZ
...
static const float4 Lock_Pos_XY_XY_A_B_D = 0;           //Position A XY B XY                            | DCC_X
static const float2 Lock_SPos_XY_C_D = 0;               //Position C XY                                 | DCC_Y
static const float4 Lock_Menu_Tresh_n_WC_D = 1000;      //Simple Menu Tresh For A,B,C & D               | DCC_Z

//Menu Detection
static const int Menu_Detection_Direction_D = 0;        // Off 0 | 1 | 2 | 3 | 4                        | MDD
static const int Menu_Cutoff_D = 0;                     // Off 0 | 1                                    | COD
...
static const float4 Match_Threshold_D = 1000.0;         //Match Threshold                               | DJ_Z
...
static const float4 Simple_Menu_Tresh_AB_D = 1000;      //Simple Menu Tresh For A & B                   | DO_W
...
static const float4 Simple_Menu_Tresh_CD_D = 1000;      //Simple Menu Tresh For C & D                   | DP_W
//Sub Block 2
...
static const float4 Simple_Menu_Tresh_EF_D = 1000;      //Simple Menu Tresh For E & F                   | DQ_W
...
static const float4 Simple_Menu_Tresh_GH_D = 1000;      //Simple Menu Tresh For G & H                   | DR_W
//Sub Block 3
...
static const float4 Simple_Menu_Tresh_IJ_D = 1000;      //Simple Menu Tresh For I & J                   | DU_W
...
static const float4 Simple_Menu_Tresh_KL_D = 1000;      //Simple Menu Tresh For K & L                   | DV_W
//Sub Block 4
...
static const float4 Simple_Menu_Tresh_MN_D = 1000;      //Simple Menu Tresh For M & N                   | DX_W
...
static const float4 Simple_Menu_Tresh_OP_D = 1000;      //Simple Menu Tresh For O & P                   | DY_W
//Simple Menu Detection A & B
static const int Simple_Menu_Detection_D = 0;           // Off 0 | 1 | 2                                | SMD
static const float4 A_SPos_XY_XY_A_B_D = 0;             //1 Position A XY B XY                          | DW_X
static const float2 A_SPos_XY_C_D = 0;                  //1 Position C XY                               | DW_Y
static const float4 A_Menu_Tresh_n_WC_D = 1000;         //1 Simple Menu Tresh For A,B,C & D             | DW_Z

static const float4 B_SPos_XY_XY_A_B_D = 0;             //2 Position A XY B XY                          | DT_X
static const float2 B_SPos_XY_C_D = 0;                  //2 Position C XY                               | DT_Y
static const float4 B_Menu_Tresh_n_WC_D = 1000;         //2 Simple Menu Tresh For A,B,C & D             | DW_W

static const float4 C_SPos_XY_XY_A_B_D = 0;             //3 Position A XY B XY                          | DAA_X
static const float2 C_SPos_XY_C_D = 0;                  //3 Position C XY                               | DAA_Y
static const float4 C_Menu_Tresh_n_WC_D = 1000;         //3 Simple Menu Tresh For A,B,C & D             | DAA_Z

static const float4 D_SPos_XY_XY_A_B_D = 0;             //4 Position A XY B XY                          | DBB_X
static const float2 D_SPos_XY_C_D = 0;                  //4 Position C XY                               | DBB_Y
static const float4 D_Menu_Tresh_n_WC_D = 1000;         //4 Simple Menu Tresh For A,B,C & D             | DBB_Z

//Stencil UI & Detection
static const int Stencil_UI_Detection_D = 0;            //Off 0 | 1 Stencil One | 2 Stencil Two         | SUI

static const int Stencil_Shape_Alpha_D = 0;             //Off 0 | 1 Square | 2 Circle                   | SSA
static const int Stencil_A_Trigger_D = 0;               //Off 0 | 1 Trigger on Aim                      | SNA
static const float4 UI_A_SPos_XY_XY_A_B_D = 0;          //Pos A = XY Any & B = ZW Lock                  | DDD_X
static const float4 UI_A_SPos_XY_C_n_XY_Pos_D = 0;      //Pos C = XY Any & UI Postion                   | DDD_Y
static const float4 UI_A_Menu_Tresh_n_WC_D = 1000;      //Simple UI Menu Tresh For A,B,C & D            | DDD_Z
static const float4 UI_A_Stencil_Adjust_D = 0;          //Stencil XY Size Z Inv & W Depth Auto-Manual   | DDD_W

static const int Stencil_Shape_Beta_D = 0;              //Off 0 | 1 Square | 2 Circle                   | SSB
static const int Stencil_B_Trigger_D = 0;               //Off 0 | 1 Trigger on Aim                      | SNB
static const float4 UI_B_SPos_XY_XY_A_B_D = 0;          //Pos A = XY Any & B = ZW Lock                  | DEE_X
static const float4 UI_B_SPos_XY_C_n_XY_Pos_D = 0;      //Pos C = XY Any & UI Postion                   | DEE_Y
static const float4 UI_B_Menu_Tresh_n_WC_D = 1000;      //Simple UI Menu Tresh For A,B,C & D            | DEE_Z
static const float4 UI_B_Stencil_Adjust_D = 0;          //Stencil XY Size Z Inv & W Depth Auto-Manual   | DEE_W
...
static const int Weapon_Boundary_Switch_D = 0;          //Weapon Boundary Switch                        | WBS
...
static const int Text_Menu_Detection_D = 0;             // Off 0 | 1 Normal | 2 Inverse | 3 Mirroed     | TMD
static const float4 TPos_XY_XY_A_B_D = 0;               //Text Position A XY B XY                       | DZ_X
...
static const float Isolating_Weapon_Stencil_Amount_D = 0; //Isolating Weapon Stencil Amount             | IWS_A
...
static const int SM_Foveated_Mode_D = 1;                //Enable or disable Foviated Rendering          | FRM
static const int Lower_Height_Adjust_D = 1;             //Lower Height Adjustment [0.5]to[0.575]        | LHA
...
//#define DG_Z 0.288 // This works. But, may be a bit overboard. Use this if users complain about edge pop out issues. I don't think it's needed.
    ...
    #define DJ_Z float4( 1000, 1000, 1000,0);           //Set Match Tresh 1000 is off
	#define BMT 1
	#define DF_Z 0.1
    #define SMS 3      //SM Toggle Separation
	#define DL_X 0.550 //SM Tune
...
//    #define DD_Z 0.007
    ...
    #define DJ_Z float4( 1000., 1000., 1000,0);                //Set Match Tresh 
...
//	#define DE_W 0.0 //Max
    ...
    #define DJ_Z float4( 26.0, 30.0, 30.0,0.0);              //Set Match Tresh   
    ...
    #define DJ_Z float4( 30., 23., 1000.,0.0);                //Set Match Tresh 
...
//	 
    ...
    #define DJ_Z float4( 1000., 1000., 1000.,0.0);            //Set Match Tresh 1000 is off
...
//	#define DG_W -0.100 //Pop
    ...
    #define DJ_Z float4( 30., 1000., 1000.,0.0);              //Set Match Tresh 
    #define MMD 3 //Set Multi Menu Detection             //Off / On
...
//	#define DK_W 2
   ...
   // #define DG_Z 0//0.05 //Min
    ...
    #define DJ_Z float4( 1000.  , 1000. , 1000.,0.0);
	//#define LBR 1
...
*/
   ...
   //Equipment Inventory Character Journal Settings and Quit menu
    ...
    #define DJ_Z float4( 0.0, 1000.0, 1000.,0.0)           //Set Match Tresh 
...
//Simple Menu Detection May add one more later
  ...
  //Multi Menu Detection                                 //4 Blocks total and 2 sub blocks per one block.
	...
	#define DF_Y 0.00         // Seperation
	#define DA_Y 250.0//410.0//200.0         // Near Plane Adjustment
    #define DA_Z -0.375      // Linerzation Offset
    #define DS_Y 2            // Linerzation Offset Effects only distance if true
	#define DB_Z 0.05         // Auto Depth Protection
	#define DE_X 4            // ZPD Boundary 
   ...
   //Simple Menu Detection May add one more later
    ...
    #define DT_X float4( 0.370 , 0.250 , 0.500 , 0.500)    //Pos A = XY Any & B = ZW Lock 
    #define DT_Y float2( 0.370 , 0.700 )                   //Pos C = XY 
    #define DW_W float4( 21.0, 0.0, 21.0, 1000.0)   //Menu Detection Type for A = X, B = Y, & C = Z. The Last Value is a Wild Card amount W is for X and Z. 
    ...
    #define DDD_X float4( 0.700 , 0.339 , 0.447 , 0.447)  //Pos A = XY Any & B = ZW Lock 
    #define DDD_Y float4( 0.935 , 0.854 , 0.690 , 0.500)      //Pos C = XY Any & UI Postion 
    #define DDD_Z float4( 14.0, 0.0, 0.0, 13.0) //Detection Value 0-30 for A = X, B = Y, & C = Z. The Last Value is a Wild Card amount W is for X and Z. 
    #define DDD_W float4( 0.240, 0.125, 1.0, 1.0)             //X is Hoz Size and Y is Vert Size, Z is Inversion amount and W is Depth set elevation with 0.0-0.5 > is Auto.

    #define SSB 1 //Off 0 | 1 Square | 2 Circle
    #define SNB 0 //Off 0 | 1 Trigger on Aim
    #define DEE_X float4( 0.500 , 0.881 , 0.500 , 0.195)  //Pos A = XY Any & B = ZW Lock 
    #define DEE_Y float4( 0.500 , 0.813 , 0.5 , 0.5)      //Pos C = XY Any & UI Postion 
    #define DEE_Z float4( 17.0, 0.0, 0.0, 18.0) //Detection Value 0-30 for A = X, B = Y, & C = Z. The Last Value is a Wild Card amount W is for X and Z. 
    #define DEE_W float4( 0.290, 0.11, 1.0, 0.625)             //X is Hoz Size and Y is Vert Size, Z is Inversion amount and W is Depth set elevation with 0.0-0.5 > is Auto.
	...
	//#define DG_W -0.250        // Shift Boundary Out of screen 0.5 and or In screen -0.5
	#define OIL 3           // Set How many Levels We use for RE_Fix 0 | 1 | 2 | 3 if 1 then it's float2(0,0) for OIF and DI_W
    #define OIF float4(0.25,0.1375,0.0625,0.00625)// Fix enables if Value is > 0.0 
	#define DI_W float4(0.5,1.5,5.0,37.5) // Like Shift Boundary DG_W But 0 to inf
	//#define FTM 0             // Fast Trigger Mode If this enabled then Level 1 and > switches instantly.
    //#define DG_Z 0.050        // Min Weapon Hands That are apart of world with Auto and Trim
    //#define DS_X float3(0.025,0,1)// Min Weapon bit only triggers when a OIL Level is set and set here on .y
    //#define DE_W 0.250        // Auto
    ...
    #define SSA 1 //Off 0 | 1 Square | 2 Circle
    #define SNA 1 //Off 0 | 1 Trigger on Aim
    #define DDD_X float4( 0.332 , 0.396 , 0.500 , 0.625)  //Pos A = XY Any & B = ZW Lock 
    #define DDD_Y float4( 0.695 , 0.385 , 0.5 , 0.5)      //Pos C = XY Any & UI Postion 
    #define DDD_Z float4( 30.0, 27.0, 7.0, 1000.0) //Detection Value 0-30 for A = X, B = Y, & C = Z. The Last Value is a Wild Card amount W is for X and Z. 
    #define DDD_W float4( 0.280, 0.365, 1.0, 0.0)             //X is Hoz Size and Y is Vert Size, Z is Inversion amount and W is Depth set elevation with 0.0-0.5 > is Auto.
/*
    ...
    #define DEE_W float4( 0.0, 0.0, 1.0, 0.0)             //X is Hoz Size and Y is Vert Size, Z is Inversion amount and W is Depth set elevation with 0.0-0.5 > is Auto.
*/
    ...
    #define DT_X float4( 0.180 , 0.482 , 0.3375, 0.0235)    //Pos A = XY Any & B = ZW Lock 
    #define DT_Y float2( 0.685 , 0.198 )                   //Pos C = XY 
    #define DW_W float4( 30.0, 0.0, 29.0, 1000.0)   //Menu Detection Type for A = X, B = Y, & C = Z. The Last Value is a Wild Card amount W is for X and Z. 

    #define DAA_X float4( 0.150 , 0.500 , 0.500 , 0.150)    //Pos A = XY Any & B = ZW Lock 
    #define DAA_Y float2( 0.500 , 0.85 )                   //Pos C = XY 
    #define DAA_Z float4( 0.0, 0.0, 0.0, 1000.0)   //Menu Detection Type for A = X, B = Y, & C = Z. The Last Value is a Wild Card amount W is for X and Z. 
	...
	#define DA_X 0.025      // ZPD
	#define DF_Y 0.00         // Seperation
	#define DA_Y 31.5         // Near Plane Adjustment
    #define DA_Z -0.25      // Linerzation Offset
    #define DS_Y 3            // Linerzation Offset Effects only distance if true
	#define DB_Z 0.05         // Auto Depth Protection
	#define DE_X 4            // ZPD Boundary 
	...
	//#define FTM 0             // Fast Trigger Mode If this enabled then Level 1 and > switches instantly.
	#define WND 0.5         //Weapon Near Pushes depth in and adjust perspective to match.
    #define DG_Z 0.035//0.025        // Min Weapon Hands That are apart of world with Auto and Trim
    //#define DS_X float3(0.025,0,1)// Min Weapon bit only triggers when a OIL Level is set and set here on .y
    #define DE_W 0.50        // Auto
    #define DI_Z 0.07 //0.100        // Trim
    #define DF_W float4(0.0001,0.00375,0.1,0.005)// Edge & Scale //0.005
	#define BMT 1             // ZPD and World Scale Balance // I need to phase this out.
    ...
    #define LHA 1
	//#define WHM 1 //Weapon Hand Masking lets you use DT_Z 
	//#define DT_Z -2.5 //Masking Power
	//#define DT_W float2(-0.25,0.015)//WH scale and cutoff	
    //Lock Out Menu Detectors  
	#define LMD 1 //Off 0 | 1
    #define DCC_X float4( 0.033 , 0.949 , 0.029, 0.949)  //Pos A = XY Any & B = ZW Lock 
    #define DCC_Y float2( 0.042 , 0.949 )                 //Pos C = XY 
    #define DCC_Z float4( 20.0, 0.0, 25.0, 1000.0) //Menu Detection Type for A = X, B = Y, & C = Z. The Last Value is a Wild Card amount W is for X and Z. 

	//Simple Menu Detection
    #define SMD 1 //Off 0 | 1 | 2 | 3 | 4 
    #define DW_X float4( 0.745, 0.0535 , 0.380 , 0.879)  //Pos A = XY Any & B = ZW Lock 
    #define DW_Y float2( 0.502, 0.880 )                 //Pos C = XY 
    #define DW_Z float4( 17.0, 0.0, 21.0, 1000.0) //Menu Detection Type for A = X, B = Y, & C = Z. The Last Value is a Wild Card amount W is for X and Z. 
    ...
    #define DO_Y float4( 0.862 , 0.822 , 0.630 , 0.920 ) //Pos A3 = XY Color & B1 = ZW Color
    #define DO_Z float4( 0.732 , 0.856 , 0.445 , 0.137) //Pos B2 = XY Black & B3 = ZW Color
	#define DO_W float4( 25.0, 23.0, 23.0, 25.0) //Tresh Hold for Color A & B and Color
	...
	//#define DG_W 0.250        // Shift Boundary Out of screen 0.5 and or In screen -0.5
	#define OIL 3           // Set How many Levels We use for RE_Fix 0 | 1 | 2 | 3 if 1 then it's float2(0,0) for OIF and DI_W
    #define OIF float4(0.375,0.250,0.125,0.0375)// Fix enables if Value is > 0.0 
	#define DI_W float4(1.25,2.5,5.0,25) // Like Shift Boundary DG_W But 0 to inf
	//#define FTM 0             // Fast Trigger Mode If this enabled then Level 1 and > switches instantly.
    ...
    #define MDD 2 //Set Menu Detection & Direction        //Off | 1 Left | 2 Right | 3 Bottom | 4 Top 
	#define COD 0 //Enable or Disable Cut-Off Direction   //Off | 1 Top & Bottom for One & Two or Left and Right for Three & Four 
    #define DN_X float4( 0.609 , 0.2325 , 0.948 , 0.500)   //Pos A = XY Any & B = ZW Lock 
    #define DN_Y float4( 0.939 , 0.721 ,  0.0, 0.0)       //Pos C = XY Any & D = ZW Match
    #define DN_Z float4( 0.0, 0.0,  0.0, 0.0)             //Pos E = XY Match & F = ZW Match
	#define DN_W float4( 0.6, 0.0 , 0.0, 0.0 )            //Size = Menu [ABC] D E F
    #define DJ_Y float4( 30.0, 0.0, 30.0, 1000.0)        //Menu Detection Type for A = X, B = Y, & C = Z. The Last Value is a Wild Card amount W is for X and Z. 
    #define DJ_Z float4( 1000., 1000., 1000, 0.0)         //Set Match Tresh and IU Y Elevation or Cut-Off -1-1

	//Simple Menu Detection
    #define SMD 2 //Off 0 | 1 | 2 | 3 | 4 
    #define DW_X float4( 0.468 , 0.172 , 0.950 , 0.890)    //Pos A = XY Any & B = ZW Lock 
    #define DW_Y float2( 0.968 , 0.172 )                   //Pos C = XY 
    #define DW_Z float4( 30.0, 0.0, 30.0, 1000.0)   //Menu Detection Type for A = X, B = Y, & C = Z. The Last Value is a Wild Card amount W is for X and Z. 
    
    #define DT_X float4( 0.662 , 0.253 , 0.765 , 0.4085)    //Pos A = XY Any & B = ZW Lock 
    #define DT_Y float2( 0.863 , 0.253 )                   //Pos C = XY 
    #define DW_W float4( 30.0, 30.0, 30.0, 1000.0)   //Menu Detection Type for A = X, B = Y, & C = Z. The Last Value is a Wild Card amount W is for X and Z. 

    #define DAA_X float4( 0.144 , 0.174 , 0.982 , 0.954)    //Pos A = XY Any & B = ZW Lock 
    #define DAA_Y float2( 0.556 , 0.174 )                   //Pos C = XY 
    #define DAA_Z float4( 21.0, 1000.0, 21.0, 1000.0)   //Menu Detection Type for A = X, B = Y, & C = Z. The Last Value is a Wild Card amount W is for X and Z. 
	...
	#define COD 0 //Enable or Disable Cut-Off Direction   //Off | 1 Top & Bottom for One & Two or Left and Right for Three & Four 
    #define DN_X float4( 0.582 , 0.117 , 0.079 , 0.935)   //Pos A = XY Any & B = ZW Lock 
    #define DN_Y float4( 0.726 , 0.117 ,  0.0, 0.0)       //Pos C = XY Any & D = ZW Match
    #define DN_Z float4( 0.0, 0.0,  0.0, 0.0)             //Pos E = XY Match & F = ZW Match
	#define DN_W float4( 1.0, 0.0 , 0.0, 0.0 )            //Size = Menu [ABC] D E F
    #define DJ_Y float4( 30.0, 3.0, 30.0, 1000.0)        //Menu Detection Type for A = X, B = Y, & C = Z. The Last Value is a Wild Card amount W is for X and Z. 
    #define DJ_Z float4( 1000., 1000., 1000, 0.0)         //Set Match Tresh and IU Y Elevation or Cut-Off -1-1
	...
	//Objective 
    #define DAA_X float4( 0.270 , 0.117 , 0.079 , 0.935)    //Pos A = XY Any & B = ZW Lock 
    #define DAA_Y float2( 0.411 , 0.117 )                   //Pos C = XY 
    #define DAA_Z float4( 30.0, 3.0, 30.0, 1000.0)   //Menu Detection Type for A = X, B = Y, & C = Z. The Last Value is a Wild Card amount W is for X and Z. 
	//Collectibles
    #define DBB_X float4( 0.427 , 0.117 , 0.079 , 0.935)    //Pos A = XY Any & B = ZW Lock 
    #define DBB_Y float2( 0.568 , 0.117 )                   //Pos C = XY 
    #define DBB_Z float4( 30.0, 3.0, 30.0, 1000.0)   //Menu Detection Type for A = X, B = Y, & C = Z. The Last Value is a Wild Card amount W is for X and Z. 
    ...
    //Simple Menu Detection May add one more later
    #define SMD 3 //Off 0 | 1 | 2 
    //Book Pop Up
    #define DW_X float4( 0.075 , 0.131 , 0.842 , 0.918)  //Pos A = XY Any & B = ZW Lock 
    #define DW_Y float2( 0.907 , 0.129 )                 //Pos C = XY 
    #define DW_Z float4( 28.0, 22.0, 30.0, 1000.0)//Menu Detection Type for A = X, B = Y, & C = Z. The Last Value is a Wild Card amount W is for X and Z. 
	//Pause Menu & Options
    #define DT_X float4( 0.075 , 0.131 , 0.842 , 0.9205)  //Pos A = XY Any & B = ZW Lock 
    #define DT_Y float2( 0.181 , 0.0495 )                 //Pos C = XY 
    #define DW_W float4( 28.0,22.0, 22.0, 1000.0)//Menu Detection Type for A = X, B = Y, & C = Z. The Last Value is a Wild Card amount W is for X and Z. 
	//Menu
    #define DAA_X float4( 0.908 , 0.129 , 0.654 , 0.899)    //Pos A = XY Any & B = ZW Lock 
    #define DAA_Y float2( 0.842 , 0.921 )                   //Pos C = XY 
    #define DAA_Z float4( 30.0, 15.0, 22.0, 1000.0)   //Menu Detection Type for A = X, B = Y, & C = Z. The Last Value is a Wild Card amount W is for X and Z. 
	...
	//Map / Book Pop Up
    #define DO_X float4( 0.613 , 0.8645, 0.500 , 0.850 ) //Pos A1 = XY Color & A2 = ZW Black 
    #define DO_Y float4( 0.702 , 0.869 , 0.613 , 0.8645 ) //Pos A3 = XY Color & B1 = ZW Color
    #define DO_Z float4( 0.500 , 0.850 , 0.702 , 0.869 ) //Pos B2 = XY Black & B3 = ZW Color
	#define DO_W float4( 29.0, 22.0, 22.0, 22.0) //Tresh Hold for Color A & B and Color
	//Pause Menu & Options / More Map
    #define DP_X float4( 0.075 , 0.131 , 0.842 , 0.918 ) //Pos C1 = XY Color & C2 = ZW Black 
    #define DP_Y float4( 0.907 , 0.129 , 0.075 , 0.132 ) //Pos C3 = XY Color & D1 = ZW Color
    #define DP_Z float4( 0.500 , 0.200 , 0.875 , 0.921 ) //Pos D2 = XY Black & D3 = ZW Color
	#define DP_W float4( 28.0, 30.0, 29.0, 22.0) //Tresh Hold for Color C & D and Color
    ...
    //Simple Menu Detection May add one more later
    #define SMD 3 //Off 0 | 1 | 2 
    #define DW_X float4( 0.382, 0.202 , 0.7015 , 0.868)  //Pos A = XY Any & B = ZW Lock 
    ...
    #define DT_X float4( 0.058, 0.143 , 0.893 , 0.943)  //Pos A = XY Any & B = ZW Lock 
    ...
    #define DAA_X float4( 0.058 , 0.144 , 0.8715 , 0.935)    //Pos A = XY Any & B = ZW Lock 
    #define DAA_Y float2( 0.162 , 0.044 )                   //Pos C = XY 
    #define DAA_Z float4( 22.0, 0.0, 22.0, 1000.0)   //Menu Detection Type for A = X, B = Y, & C = Z. The Last Value is a Wild Card amount W is for X and Z. 
/*
    #define DBB_X float4( 0.8835, 0.956 , 0.982 , 0.954)    //Pos A = XY Any & B = ZW Lock 
    #define DBB_Y float2( 0.500 , 0.004 )                   //Pos C = XY 
    #define DBB_Z float4( 1000.0, 1000.0, 1000.0, 1000.0)   //Menu Detection Type for A = X, B = Y, & C = Z. The Last Value is a Wild Card amount W is for X and Z. 
*/
   ...
   //#define DS_Z 2            // Set View Mode
	...
	#define OIL 2           // Set How many Levels We use for RE_Fix 0 | 1 | 2 | 3 if 1 then it's float2(0,0) for OIF and DI_W
    #define OIF float3(0.5,0.375,0.250)// Fix enables if Value is > 0.0 
	#define DI_W float3(1.5,2.5,3.5) // Like Shift Boundary DG_W But 0 to inf
	//#define FTM 0             // Fast Trigger Mode If this enabled then Level 1 and > switches instantly.
	#define DG_Z 0.050        // Min Weapon Hands That are apart of world with Auto and Trim
    //#define DS_X float3(0.025,0,1)// Min Weapon bit only triggers when a OIL Level is set and set here on .y
    #define DE_W 0.50        // Auto
    #define DI_Z 0.125        // Trim
	#define WND 0.5         //Weapon Near Pushes depth in and adjust perspective to match.
    #define DF_W float4(0.0001,0.0025,0.050,0.025)// Edge & Scale
	...
	//#define FTM 0             // Fast Trigger Mode If this enabled then Level 1 and > switches instantly.
	#define WND 0.500         //Weapon Near Pushes depth in and adjust perspective to match.
	...
	#define BMT 1             // ZPD and World Scale Balance // I need to phase this out.
	#define DF_Z 0.150        // Set the Balance  
    #define DL_Y 1.5       // De-Artifact Only works on some View Modes and causes performance degredation
	//#define DB_Y 1.0          // Effects De-Artifacts -1 to 1 Most of the time leave this at 0 and if you set 1 it takes depth into account 
    //#define DL_Z -1.0         // Compat Power
	#define WSM 5
	#define DB_W 27
    ...
    #define NDW 1
    #define NMW 1	
#elif (App == 0x55AB5DC5) //The Invincible -- Too many issue due to FOV/BD
    //#define DS_Z 2            // Set View Mode
	#define DA_W 1            // Set Linerzation
    //#define DB_X 1            // Flip
	#define DA_X 0.025         // ZPD
	...
	#define DA_X 0.025      // ZPD
	#define DF_Y 0.00         // Seperation
	#define DA_Y 600.0         // Near Plane Adjustment
    //#define DA_Z -0.0001      // Linerzation Offset
    #define DS_Y 2            // Linerzation Offset Effects only distance if true
	#define DB_Z 0.025         // Auto Depth Protection
	#define DE_X 7            // ZPD Boundary 
	...
	#define DG_W -0.25        // Shift Boundary Out of screen 0.5 and or In screen -0.5
	//#define OIL 1           // Set How many Levels We use for RE_Fix 0 | 1 | 2 | 3 if 1 then it's float2(0,0) for OIF and DI_W
    #define OIF 0.5//float2(0.5,0.25)// Fix enables if Value is > 0.0 
	#define DI_W 0.5//float2(0.5,2.5) // Like Shift Boundary DG_W But 0 to inf
    //#define CWH 0             //ZPD Weapon Hand Consideration For Masking  0 is Off | 1 Is Full | 2 Half Right screen Mask And Not to be used with Weapon Profiles.
    //#define WBA 1.0           //ZPD Weapon Boundary Alt Adjust power for CWH 
	//#define FTM 0             // Fast Trigger Mode If this enabled then Level 1 and > switches instantly.
	//#define WND 0.175         //Weapon Near Pushes depth in and adjust perspective to match.
	...
	//#define OIL 1           // Set How many Levels We use for RE_Fix 0 | 1 | 2 | 3 if 1 then it's float2(0,0) for OIF and DI_W
    //#define OIF float2(0.5,0.25)// Fix enables if Value is > 0.0 
	//#define DI_W float2(0.625,2.5) // Like Shift Boundary DG_W But 0 to inf
    //#define CWH 0             //ZPD Weapon Hand Consideration For Masking  0 is Off | 1 Is Full | 2 Half Right screen Mask And Not to be used with Weapon Profiles.
    //#define WBA 1.0           //ZPD Weapon Boundary Alt Adjust power for CWH 
	//#define FTM 0             // Fast Trigger Mode If this enabled then Level 1 and > switches instantly.
	#define WND 0.5         //Weapon Near Pushes depth in and adjust perspective to match.
	...
	#define DF_X float2(0.175,0)  // ZPD Weapon Boundarys Level 1 and Level 2
	//#define DJ_W 0.1	      // Weapon Depth Limit Location 1
	//#define DS_W 1.0	      // Weapon Depth Limit Location 2
	//#define WFB 0.0          // ZPD Weapon Elevaton for 1 and 2 scales from [0 - 1]
	//#define DT_W float2(0.015,0.03)//WH scale and cutoff
	...
	#define DA_X 0.025      // ZPD
	#define DF_Y 0.025         // Seperation
	#define DA_Y 800.0         // Near Plane Adjustment
    //#define DA_Z -0.0001      // Linerzation Offset
    #define DS_Y 2            // Linerzation Offset Effects only distance if true
	#define DB_Z 0.03         // Auto Depth Protection
	#define DE_X 4            // ZPD Boundary 
	#define DE_Y 0.750        // Set ZPD Boundary Level Zero 
	#define DE_Z 0.375        // Speed that Boundary is Enforced
	//#define AFD 1           // Alternate Frame Detection - May be phased out
	...
	#define AWZ 1.0          // Anti-Weapon Hand Z-Fighting -1 is shifted to the right a little bit and 1 is center
	#define WSM 6             // Weapon Setting Mode 
	#define DB_W 4           // Weapon Profile
	#define DF_X float2(0.15,0)  // ZPD Weapon Boundarys Level 1 and Level 2
	//#define DJ_W 0.1	      // Weapon Depth Limit Location 1
	//#define DS_W 1.0	      // Weapon Depth Limit Location 2
	//#define WFB 0.0          // ZPD Weapon Elevaton for 1 and 2 scales from [0 - 1]
	//#define DT_W float2(0.015,0.03)//WH scale and cutoff
    ...
    //#define DM_Y 3     //HQ VRS 
	#define FOV 1
#elif (App == 0xEFD03791 ) //Diva Mega Mix
    //#define DS_Z 2            // Set View Mode
	#define DA_W 1            // Set Linerzation
    //#define DB_X 1            // Flip
	#define DA_X 0.250         // ZPD
	...
	#define DE_X 1            // ZPD Boundary 
	#define DE_Y 0.750        // Set ZPD Boundary Level Zero 
	#define DE_Z 0.3875        // Speed that Boundary is Enforced
	//#define AFD 1           // Alternate Frame Detection - May be phased out
	#define DG_W 0.125        // Shift Boundary Out of screen 0.5 and or In screen -0.5
	#define OIL 3           // Set How many Levels We use for RE_Fix 0 | 1 | 2 | 3 if 1 then it's float2(0,0) for OIF and DI_W
    #define OIF float4(0.5,0.35,0.225,0.075)// Fix enables if Value is > 0.0 
	#define DI_W float4(0.5,1.0,2.0,5.0) // Like Shift Boundary DG_W But 0 to inf
    //#define CWH 0             //ZPD Weapon Hand Consideration For Masking  0 is Off | 1 Is Full | 2 Half Right screen Mask And Not to be used with Weapon Profiles.
    //#define WBA 1.0           //ZPD Weapon Boundary Alt Adjust power for CWH 
	//#define FTM 0             // Fast Trigger Mode If this enabled then Level 1 and > switches instantly.
	#define WND 1.0         //Weapon Near Pushes depth in and adjust perspective to match.
    ...
    #define DE_W 0.250        // Auto
    #define DI_Z 0.1       // Trim
    #define DF_W float4(0.0001,0,0.0,0.05)// Edge & Scale
	#define BMT 1             // ZPD and World Scale Balance // I need to phase this out.
	#define DF_Z 0.250        // Set the Balance  
    //#define DL_Y 0.50         // De-Artifact Only works on some View Modes and causes performance degredation
	//#define DB_Y 1.0          // Effects De-Artifacts -1 to 1 Most of the time leave this at 0 and if you set 1 it takes depth into account 
  ...
  //Simple Menu Detection May add one more later
    ...
    #define DW_X float4( 0.310, 0.055 , 0.375 , 0.400)  //Pos A = XY Any & B = ZW Lock 
    #define DW_Y float2( 0.688 , 0.055 )                 //Pos C = XY 
    #define DW_Z float4( 23.0, 0.0, 23.0, 1000.0)//Menu Detection Type for A = X, B = Y, & C = Z. The Last Value is a Wild Card amount W is for X and Z. 
	...
	#define DB_W 20
	#define RHW 1
#elif (App == 0x6ACBFF45 ) //Forsaken Remastred
    //#define DS_Z 2            // Set View Mode
	//#define DA_W 1            // Set Linerzation
    //#define DB_X 1            // Flip
	...
	#define DB_Z 0.050         // Auto Depth Protection
	#define DE_X 1            // ZPD Boundary 
	#define DE_Y 0.8750        // Set ZPD Boundary Level Zero 
	#define DE_Z 0.375        // Speed that Boundary is Enforced
	//#define AFD 1           // Alternate Frame Detection - May be phased out
	//#define DG_W -0.125        // Shift Boundary Out of screen 0.5 and or In screen -0.5
	#define OIL 3           // Set How many Levels We use for RE_Fix 0 | 1 | 2 | 3 if 1 then it's float2(0,0) for OIF and DI_W
    #define OIF float4(0.75,0.625,0.5,0.25)// Fix enables if Value is > 0.0 
	#define DI_W float4(0.25,0.5,1.0,2.0) // Like Shift Boundary DG_W But 0 to inf
    //#define CWH 2             //ZPD Weapon Hand Consideration For Masking  0 is Off | 1 Is Full | 2 Half Right screen Mask And Not to be used with Weapon Profiles.
    //#define WBA 2.0           //ZPD Weapon Boundary Alt Adjust power for CWH 
	//#define FTM 0             // Fast Trigger Mode If this enabled then Level 1 and > switches instantly.
	#define WND 0.5         //Weapon Near Pushes depth in and adjust perspective to match.
	...
	#define DA_X 0.025         // ZPD
	#define DF_Y 0.025         // Seperation
	#define DA_Y 195.0         // Near Plane Adjustment
    //#define DA_Z -0.0001      // Linerzation Offset
    #define DS_Y 2            // Linerzation Offset Effects only distance if true
	#define DB_Z 0.025         // Auto Depth Protection
	#define DE_X 5            // ZPD Boundary 
	...
	//#define DG_W -0.125        // Shift Boundary Out of screen 0.5 and or In screen -0.5
	#define OIL 3           // Set How many Levels We use for RE_Fix 0 | 1 | 2 | 3 if 1 then it's float2(0,0) for OIF and DI_W
    #define OIF float4(0.625,0.50,0.25,0.075)// Fix enables if Value is > 0.0 
	#define DI_W float4(0.25,0.75,5.0,7.5) // Like Shift Boundary DG_W But 0 to inf
    //#define CWH 2             //ZPD Weapon Hand Consideration For Masking  0 is Off | 1 Is Full | 2 Half Right screen Mask And Not to be used with Weapon Profiles.
    //#define WBA 2.0           //ZPD Weapon Boundary Alt Adjust power for CWH 
	//#define FTM 0             // Fast Trigger Mode If this enabled then Level 1 and > switches instantly.
	#define WND 0.25         //Weapon Near Pushes depth in and adjust perspective to match.
    ...
    //#define DL_Z 1.00         // Compat Power
	//#define DJ_X 0.250        // Range Smoothing
	#define AWZ 2          // Anti-Weapon Hand Z-Fighting -1 is shifted to the right a little bit and 1 is center
	#define WSM 4             // Weapon Setting Mode 
    ...
    //#define DB_X 1            // Flip
	#define DA_X 0.025         // ZPD
	#define DF_Y 0.0           // Seperation
	#define DA_Y 300.0         // Near Plane Adjustment
    //#define DA_Z -0.0001      // Linerzation Offset
    #define DS_Y 2            // Linerzation Offset Effects only distance if true
	#define DB_Z 0.03         // Auto Depth Protection
	#define DE_X 5            // ZPD Boundary 
	...
	//#define DG_W -0.125        // Shift Boundary Out of screen 0.5 and or In screen -0.5
	#define OIL 1           // Set How many Levels We use for RE_Fix 0 | 1 | 2 | 3 if 1 then it's float2(0,0) for OIF and DI_W
    #define OIF float2(0.6,0.5)// Fix enables if Value is > 0.0 
	#define DI_W float2(0.25,0.5) // Like Shift Boundary DG_W But 0 to inf
    //#define CWH 2             //ZPD Weapon Hand Consideration For Masking  0 is Off | 1 Is Full | 2 Half Right screen Mask And Not to be used with Weapon Profiles.
    //#define WBA 2.0           //ZPD Weapon Boundary Alt Adjust power for CWH 
	//#define FTM 0             // Fast Trigger Mode If this enabled then Level 1 and > switches instantly.
	//#define WND -0.5         //Weapon Near Pushes depth in and adjust perspective to match.
	...
	#define OIL 1           // Set How many Levels We use for RE_Fix 0 | 1 | 2 | 3 if 1 then it's float2(0,0) for OIF and DI_W
    #define OIF float2(0.5,0.25)// Fix enables if Value is > 0.0 
	#define DI_W float2(1.0,5.0) // Like Shift Boundary DG_W But 0 to inf
    //#define CWH 2             //ZPD Weapon Hand Consideration For Masking  0 is Off | 1 Is Full | 2 Half Right screen Mask And Not to be used with Weapon Profiles.
    //#define WBA 2.0           //ZPD Weapon Boundary Alt Adjust power for CWH 
	//#define FTM 0             // Fast Trigger Mode If this enabled then Level 1 and > switches instantly.
	#define WND -0.5         //Weapon Near Pushes depth in and adjust perspective to match.
	...
	#define BMT 1             // ZPD and World Scale Balance // I need to phase this out.
	#define DF_Z 0.15        // Set the Balance  
    #define DL_Y 2.0         // De-Artifact Only works on some View Modes and causes performance degredation
	//#define DB_Y 2.0          // Effects De-Artifacts -1 to 1 Most of the time leave this at 0 and if you set 1 it takes depth into account 
    //#define DL_Z 1.00         // Compat Power
	//#define DJ_X 0.250        // Range Smoothing
	//#define AWZ -1          // Anti-Weapon Hand Z-Fighting -1 is shifted to the right a little bit and 1 is center
	//#define WSM 4             // Weapon Setting Mode 
	...
	//#define DG_W -0.125        // Shift Boundary Out of screen 0.5 and or In screen -0.5
	#define OIL 3           // Set How many Levels We use for RE_Fix 0 | 1 | 2 | 3 if 1 then it's float2(0,0) for OIF and DI_W
    #define OIF float4(0.5,0.375,0.25,0.0625)// Fix enables if Value is > 0.0 
	#define DI_W float4(0.75,1.5,2.0,5.0) // Like Shift Boundary DG_W But 0 to inf
    #define CWH 1             //ZPD Weapon Hand Consideration For Masking  0 is Off | 1 Is Full | 2 Half Right screen Mask And Not to be used with Weapon Profiles.
    #define WBA 1.0           //ZPD Weapon Boundary Alt Adjust power for CWH 
	//#define FTM 0             // Fast Trigger Mode If this enabled then Level 1 and > switches instantly.
	#define WND 0.5         //Weapon Near Pushes depth in and adjust perspective to match.
	...
	#define PEW 1
	#define DSW 1
#elif (App == 0x4EEE961F ) //Hollow Cocoon
    //#define DS_Z 2            // Set View Mode
	#define DA_W 1            // Set Linerzation
    #define DB_X 1            // Flip
	#define DA_X 0.025         // ZPD
	#define DF_Y 0.0125           // Seperation
	#define DA_Y 43.5         // Near Plane Adjustment
    //#define DA_Z -0.5        // Linerzation Offset
    #define DS_Y 2            // Linerzation Offset Effects only distance if true
	...
	#define OIL 3           // Set How many Levels We use for RE_Fix 0 | 1 | 2 | 3 if 1 then it's float2(0,0) for OIF and DI_W
    #define OIF float4(0.5,0.375,0.250,0.125)// Fix enables if Value is > 0.0 
	#define DI_W float4(0.5,1.5,2.0,3.75) // Like Shift Boundary DG_W But 0 to inf
    //#define CWH 1             //ZPD Weapon Hand Consideration For Masking  0 is Off | 1 Is Full | 2 Half Right screen Mask And Not to be used with Weapon Profiles.
    //#define WBA 1.0           //ZPD Weapon Boundary Alt Adjust power for CWH 
	//#define FTM 0             // Fast Trigger Mode If this enabled then Level 1 and > switches instantly.
	#define WND 1.0         //Weapon Near Pushes depth in and adjust perspective to match.
    ...
    //Menu Detection Templates -  Needs a Specail Shader to adjust
    #define MDD 1 //Set Menu Detection & Direction      //Off | 1 | 2 | 3 | 4      
    #define DN_X float4( 0.038 , 0.961 , 0.482 , 0.816) //Pos A = XY Any & B = ZW Lock 
    #define DN_Y float4( 0.945 , 0.965 ,  0.0, 0.0)     //Pos C = XY Any & D = ZW Match
    #define DN_Z float4( 0.0, 0.0,  0.0, 0.0)           //Pos E = XY Match & F = ZW Match
	#define DN_W float4( 1.0, 0.0 , 0.0, 0.0 )          //Size = Menu [ABC] D E F
    #define DJ_Y float4( 25.0, 30.0, 24.0, 1000.0)       //Menu Detection Type for A = X, B = Y, & C = Z. The Last Value is a Wild Card amount W is for X and Z. 
    #define DJ_Z float4( 1000., 1000., 1000.,0.0)           //Set Match Tresh 
	
	//Simple Menu Detection May add one more later
    #define SMD 2 //Off 0 | 1 | 2 
    #define DW_X float4( 0.068 , 0.130 , 0.450 , 0.140)  //Pos A = XY Any & B = ZW Lock 
    #define DW_Y float2( 0.950 , 0.965 )                 //Pos C = XY 
    #define DW_Z float4( 24.0, 24.0, 24.0, 1000.0)//Menu Detection Type for A = X, B = Y, & C = Z. The Last Value is a Wild Card amount W is for X and Z. 
    
    #define DT_X float4( 0.402 , 0.120 , 0.883 , 0.140 )  //Pos A = XY Any & B = ZW Lock 
    #define DT_Y float2( 0.950 , 0.965 )                 //Pos C = XY 
    #define DW_W float4( 24.0, 24.0, 24.0, 1000.0)//Menu Detection Type for A = X, B = Y, & C = Z. The Last Value is a Wild Card amount W is for X and Z. 

    //Text Menu Detection
    #define TMD 1 //Off 0 | 1Normal | 2 Inverse | 3 Mirror
    #define DZ_X float4( 0.300 , 0.196 , 0.050 , 0.075)  //Pos A = XY Any & B = ZW Lock 
    #define DZ_Y float2( 0.708 , 0.238 )                 //Pos C = XY 
    #define DZ_Z float4( 30.0, 0.0, 30.0, 1000.0) //Text Detection Type for A = X, B = Y, & C = Z. The Last Value is a Wild Card amount W is for X and Z. 
    #define DZ_W float4( 0.970, 0.835, 1.0, 0.0)             //Text Distance from screen, Text Cutoff point, and adjustable XY Masking.
	...
	#define PEW 1
	#define DAA 1
#elif (App == 0x5EA0EB6E ) //The Land of Pain
    //#define DS_Z 2            // Set View Mode
	#define DA_W 1            // Set Linerzation
    //#define DB_X 1            // Flip
	#define DA_X 0.025         // ZPD
	...
	#define DB_Z 0.025         // Auto Depth Protection
	#define DE_X 5            // ZPD Boundary 
	#define DE_Y 0.725        // Set ZPD Boundary Level Zero 
	#define DE_Z 0.375        // Speed that Boundary is Enforced
	//#define AFD 1           // Alternate Frame Detection - May be phased out
	#define DG_W -0.035        // Shift Boundary Out of screen 0.5 and or In screen -0.5
	#define OIL 1           // Set How many Levels We use for RE_Fix 0 | 1 | 2 | 3 if 1 then it's float2(0,0) for OIF and DI_W
    #define OIF float2(0.625,0.5)// Fix enables if Value is > 0.0 
	#define DI_W float2(0.5,1.0) // Like Shift Boundary DG_W But 0 to inf
    //#define CWH 1             //ZPD Weapon Hand Consideration For Masking  0 is Off | 1 Is Full | 2 Half Right screen Mask And Not to be used with Weapon Profiles.
	...
	//#define DB_Y 1.0          // Effects De-Artifacts -1 to 1 Most of the time leave this at 0 and if you set 1 it takes depth into account 
    //#define DL_Z 0.1         // Compat Power
	#define DJ_X 0.0275        // Range Smoothing
	//#define AWZ -1          // Anti-Weapon Hand Z-Fighting -1 is shifted to the right a little bit and 1 is center
	#define WSM 6             // Weapon Setting Mode 
	#define DB_W 2           // Weapon Profile	
	#define WBS 1             //Weapon Boundary Switch 0 | 1
	#define DF_X float2(0.375,0)  // ZPD Weapon Boundarys Level 1 and Level 2
	//#define LBR 1
	//#define LBM 1 //Letter Box Masking With size Adust for Top and Bottom
	//#define DI_X 0.840
	//#define DI_Y 0.160
	#define DT_W float2(0.0,0.014)//WH scale and cutoff	

    //Simple Menu Detection
    #define SMD 2  
    #define DW_X float4( 0.3845, 0.019 , 0.500 , 0.030)  //Pos A = XY Any & B = ZW Lock 
    #define DW_Y float2( 0.609 , 0.026 )                 //Pos C = XY 
    #define DW_Z float4( 30.0, 0.0, 30.0, 1000.0) //Menu Detection Type for A = X, B = Y, & C = Z. The Last Value is a Wild Card amount W is for X and Z. 

    #define DT_X float4( 0.322, 0.019 , 0.500 , 0.030)  //Pos A = XY Any & B = ZW Lock 
    #define DT_Y float2( 0.665 , 0.020 )                 //Pos C = XY 
    #define DW_W float4( 30.0, 0.0, 30.0, 1000.0) //Menu Detection Type for A = X, B = Y, & C = Z. The Last Value is a Wild Card amount W is for X and Z. 
	...
	#define DF_Y 0.0125           // Seperation
	#define DA_Y 100.0         // Near Plane Adjustment
    #define DA_Z -0.0125       // Linerzation Offset
    //#define DS_Y 1            // Linerzation Offset Effects only distance if true
	#define DB_Z 0.025         // Auto Depth Protection
	#define DE_X 1            // ZPD Boundary 
	#define DE_Y 0.75        // Set ZPD Boundary Level Zero 
	...
	#define DG_W 0.5        // Shift Boundary Out of screen 0.5 and or In screen -0.5
	#define OIL 1           // Set How many Levels We use for RE_Fix 0 | 1 | 2 | 3 if 1 then it's float2(0,0) for OIF and DI_W
    #define OIF float2(0.5,0.300)// Fix enables if Value is > 0.0 
	#define DI_W float2(1.0,2.0) // Like Shift Boundary DG_W But 0 to inf
    ...
    //#define DM_Y 3     //HQ VRS 
	#define DAA 1
#elif (App == 0xBA2E23D9 ) //Narcosis
    //#define DS_Z 2            // Set View Mode
	#define DA_W 1            // Set Linerzation
    #define DB_X 1            // Flip
	#define DA_X 0.025         // ZPD
	#define DF_Y 0.0           // Seperation
	#define DA_Y 120.0         // Near Plane Adjustment
    //#define DA_Z -0.0125       // Linerzation Offset
    //#define DS_Y 1            // Linerzation Offset Effects only distance if true
	//#define DB_Z 0.025         // Auto Depth Protection
	#define DE_X 4            // ZPD Boundary 
	...
	#define OIL 1           // Set How many Levels We use for RE_Fix 0 | 1 | 2 | 3 if 1 then it's float2(0,0) for OIF and DI_W
    #define OIF float2(0.5,0.250)// Fix enables if Value is > 0.0 
	#define DI_W float2(1.0,1.0) // Like Shift Boundary DG_W But 0 to inf
  //Simple Menu Detection
    #define SMD 1 //Off 0 | 1 | 2 | 3 | 4 
    #define DW_X float4( 0.070, 0.875 , 0.500 , 0.844)  //Pos A = XY Any & B = ZW Lock 
    #define DW_Y float2( 0.858 , 0.500 )                 //Pos C = XY 
    #define DW_Z float4( 29.0, 0.0, 19.0, 1000.0) //Menu Detection Type for A = X, B = Y, & C = Z. The Last Value is a Wild Card amount W is for X and Z. 
	...
	//#define DG_W -0.0        // Shift Boundary Out of screen 0.5 and or In screen -0.5
	#define OIL 2           // Set How many Levels We use for RE_Fix 0 | 1 | 2 | 3 if 1 then it's float2(0,0) for OIF and DI_W
    #define OIF float3(0.750,0.50,0.375)// Fix enables if Value is > 0.0 
	#define DI_W float3(0.125,0.5,1.0) // Like Shift Boundary DG_W But 0 to inf
  //Simple Menu Detection
  //  #define SMD 1 //Off 0 | 1 | 2 | 3 | 4 
  //  #define DW_X float4( 0.070, 0.875 , 0.500 , 0.844)  //Pos A = XY Any & B = ZW Lock 
  //  #define DW_Y float2( 0.858 , 0.500 )                 //Pos C = XY 
  //  #define DW_Z float4( 29.0, 0.0, 19.0, 1000.0) //Menu Detection Type for A = X, B = Y, & C = Z. The Last Value is a Wild Card amount W is for X and Z. 
	...
	#define NMW 1
	#define NCW 1
#elif (App == 0xC5BE965B ) //Chronos Before The Ashes
    //#define DS_Z 2            // Set View Mode
	#define DA_W 1            // Set Linerzation
    //#define DB_X 1            // Flip
	#define DA_X 0.025         // ZPD
	...
	//#define DG_W -0.0        // Shift Boundary Out of screen 0.5 and or In screen -0.5
	#define OIL 3           // Set How many Levels We use for RE_Fix 0 | 1 | 2 | 3 if 1 then it's float2(0,0) for OIF and DI_W
    #define OIF float4(0.750,0.50,0.375,0.125)// Fix enables if Value is > 0.0 
	#define DI_W float4(0.5,1.0,2.5,4.0) // Like Shift Boundary DG_W But 0 to inf
  //Multi Menu Detection                    //4 Blocks total and 2 sub blocks per one block.
    #define MMD 2 //Set Multi Menu Detection  // Off  0 | 1 | 2 | 3 | 4
    #define MMS 0 //Set Multi Menu Selection  // Off  0 | 1 | 2 | 3 | 4                       //0-2 check to 28-30 check Depnding on the option MML
    #define MML -2//Set Multi Menu Leniency   // -4 |-3 |-2 |-1 | Off | 1 | 2 | 3 | 4         //Negitive [0 & 30] No Leniency | Zero [0-1 & 29-30] Some Leniency | Positive [0-2 & 28-30] Most Leniency    
	//Block One
    #define DO_X float4( 0.448 , 0.038 , 0.552 , 0.050 ) //Pos A1 = XY Color & A2 = ZW Black 
    #define DO_Y float4( 0.539 , 0.901 , 0.485 , 0.046 ) //Pos A3 = XY Color & B1 = ZW Color
    #define DO_Z float4( 0.552 , 0.050 , 0.539 , 0.901 ) //Pos B2 = XY Black & B3 = ZW Color
	#define DO_W float4( 30.0, 18.0, 30.0, 18.0) //Tresh Hold for Color A & B and Color

    #define DP_X float4( 0.518 , 0.051 , 0.552 , 0.050 ) //Pos C1 = XY Color & C2 = ZW Black 
    #define DP_Y float4( 0.539 , 0.901 , 0.552 , 0.061 ) //Pos C3 = XY Color & D1 = ZW Color
    #define DP_Z float4( 0.552 , 0.050 , 0.539 , 0.901 ) //Pos D2 = XY Black & D3 = ZW Color
	#define DP_W float4( 30.0, 18.0, 30.0, 18.0) //Tresh Hold for Color C & D and Color
    //Block Two
	#define DQ_X float4( 0.4485, 0.075 , 0.552 , 0.050 ) //Pos C1 = XY Color & C2 = ZW Black 
    #define DQ_Y float4( 0.539 , 0.901 , 0.475 , 0.051 ) //Pos C3 = XY Color & D1 = ZW Color
    #define DQ_Z float4( 0.552 , 0.050 , 0.539 , 0.901 ) //Pos D2 = XY Black & D3 = ZW Color
	#define DQ_W float4( 30.0, 18.0, 30.0, 18.0) //Tresh Hold for Color A1 & A3 and Color

	#define DR_X float4( 0.517 , 0.075 , 0.552 , 0.050 ) //Pos G1 = XY Color & G2 = ZW Black 
    #define DR_Y float4( 0.539 , 0.901 , 0.000 , 0.000 ) //Pos G3 = XY Color & H1 = ZW Color
    #define DR_Z float4( 0.000 , 0.000 , 0.000 , 0.000 ) //Pos H2 = XY Black & H3 = ZW Color
	#define DR_W float4( 30.0, 18.0, 1000.0, 1000.0) //Tresh Hold for Color G & H and Color 
	...
	#define DF_Z 0.100        // Set the Balance  
    #define DL_Y 0.25         // De-Artifact Only works on some View Modes and causes performance degredation
	//#define DB_Y 0.25          // Effects De-Artifacts -1 to 1 Most of the time leave this at 0 and if you set 1 it takes depth into account 
    #define DL_Z 0.25        // Compat Power
	//#define DJ_X 0.125        // Range Smoothing
	//#define WSM 6             // Weapon Setting Mode 
	//#define DB_W 6           // Weapon Profile
    ...
    #define DM_Y 4     //HQ VRS 
	#define DAA 1
#elif (App == 0x9B6F3496 ) //Bloodstauned Riturl of the Night
    #define DS_Z 2            // Set View Mode
	#define DA_W 1            // Set Linerzation
    //#define DB_X 1            // Flip
	#define DA_X 0.25          // ZPD
	#define DF_Y 0.00           // Seperation
	#define DA_Y 100.0          // Near Plane Adjustment
  ...
  //Simple Menu Detection
    #define SMD 1 //Off 0 | 1 | 2 | 3 | 4 
    #define DW_X float4( 0.506, 0.129 , 0.516 , 0.140)  //Pos A = XY Any & B = ZW Lock 
    #define DW_Y float2( 0.119 , 0.050 )                 //Pos C = XY 
    #define DW_Z float4( 28.0, 0.0, 28.0, 1000.0) //Menu Detection Type for A = X, B = Y, & C = Z. The Last Value is a Wild Card amount W is for X and Z. 
	...
	#define DA_X 0.025          // ZPD
	#define DF_Y 0.00           // Seperation
	#define DA_Y 187.5          // Near Plane Adjustment
    //#define DA_Z 0.000125       // Linerzation Offset
    #define DS_Y 1            // Linerzation Offset Effects only distance if true
	#define DB_Z 0.020         // Auto Depth Protection
	#define DE_X 1            // ZPD Boundary 
	#define DE_Y 0.800        // Set ZPD Boundary Level Zero 
	#define DE_Z 0.375        // Speed that Boundary is Enforced
	//#define AFD 1           // Alternate Frame Detection - May be phased out
	//#define DG_W -0.25        // Shift Boundary Out of screen 0.5 and or In screen -0.5
	#define OIL 3           // Set How many Levels We use for RE_Fix 0 | 1 | 2 | 3 if 1 then it's float2(0,0) for OIF and DI_W
    #define OIF float4(0.6,0.5,0.375,0.125)// Fix enables if Value is > 0.0 
	#define DI_W float4(0.25,0.5,2.5,15.0) // Like Shift Boundary DG_W But 0 to inf
	#define WND 0.5         //Weapon Near Pushes depth in and adjust perspective to match.
    #define DG_Z 0.025        // Min Weapon Hands That are apart of world with Auto and Trim
    //#define DS_X float3(0.025,0,1)// Min Weapon bit only triggers when a OIL Level is set and set here on .y
    //#define DE_W 0.50        // Auto
    ...
    #define DW_X float4( 0.500 , 0.366 , 0.500 , 0.040)  //Pos A = XY Any & B = ZW Lock 
    #define DW_Y float2( 0.872 , 0.910 )                 //Pos C = XY 
    #define DW_Z float4( 28.0, 0.0, 30.0, 1000.0) //Menu Detection Type for A = X, B = Y, & C = Z. The Last Value is a Wild Card amount W is for X and Z. 

    #define DT_X float4( 0.500 , 0.354 , 0.500 , 0.040)  //Pos A = XY Any & B = ZW Lock 
    #define DT_Y float2( 0.872 , 0.961 )                 //Pos C = XY 
    #define DW_W float4( 28.0, 0.0, 30.0, 1000.0) //Menu Detection Type for A = X, B = Y, & C = Z. The Last Value is a Wild Card amount W is for X and Z. 

    #define DAA_X float4( 0.5445, 0.185 , 0.500 , 0.040)  //Pos A = XY Any & B = ZW Lock 
    #define DAA_Y float2( 0.872 , 0.961 )                 //Pos C = XY 
    #define DAA_Z float4( 18.0, 0.0, 30.0, 1000.0) //Menu Detection Type for A = X, B = Y, & C = Z. The Last Value is a Wild Card amount W is for X and Z. 

    #define DBB_X float4( 0.3045, 0.185 , 0.500 , 0.040)  //Pos A = XY Any & B = ZW Lock 
    #define DBB_Y float2( 0.872 , 0.961 )                 //Pos C = XY 
    #define DBB_Z float4( 18.0, 0.0, 30.0, 1000.0) //Menu Detection Type for A = X, B = Y, & C = Z. The Last Value is a Wild Card amount W is for X and Z. 
  ...
  //  #define SDU 1	
	...
	#define PEW 1 
	#define DAA 1
#elif (App == 0x847C7024 ) //Talos Principle 2
    //#define DS_Z 2            // Set View Mode
	#define DA_W 1            // Set Linerzation
    //#define DB_X 1            // Flip
	#define DA_X 0.025          // ZPD
	#define DF_Y 0.00           // Seperation
	#define DA_Y 350.0          // Near Plane Adjustment
    //#define DA_Z 0.000125       // Linerzation Offset
    #define DS_Y 5            // Linerzation Offset Effects only distance if true
	#define DB_Z 0.005         // Auto Depth Protection
	#define DE_X 5            // ZPD Boundary 
	#define DE_Y 0.75        // Set ZPD Boundary Level Zero 
	#define DE_Z 0.40        // Speed that Boundary is Enforced
	//#define AFD 1           // Alternate Frame Detection - May be phased out
	//#define DG_W -0.25        // Shift Boundary Out of screen 0.5 and or In screen -0.5
	#define OIL 2           // Set How many Levels We use for RE_Fix 0 | 1 | 2 | 3 if 1 then it's float2(0,0) for OIF and DI_W
    #define OIF float3(0.625,0.5,0.3)// Fix enables if Value is > 0.0 
	#define DI_W float3(0.5,1.0,2.5) // Like Shift Boundary DG_W But 0 to inf
	#define WND 0.5         //Weapon Near Pushes depth in and adjust perspective to match.
    //#define DG_Z 0.025        // Min Weapon Hands That are apart of world with Auto and Trim
    //#define DS_X float3(0.025,0,1)// Min Weapon bit only triggers when a OIL Level is set and set here on .y
    //#define DE_W 0.50        // Auto
  ...
  //Simple Menu Detection
    #define SMD 1 //Off 0 | 1 | 2 | 3 | 4 
    #define DW_X float4( 0.107, 0.241 , 0.44 , 0.04)  //Pos A = XY Any & B = ZW Lock 
    #define DW_Y float2( 0.371 , 0.298 )                 //Pos C = XY 
    #define DW_Z float4( 27.0, 0.0, 27.0, 1000.0) //Menu Detection Type for A = X, B = Y, & C = Z. The Last Value is a Wild Card amount W is for X and Z. 
  ...
  //Multi Menu Detection                    //4 Blocks total and 2 sub blocks per one block.
	...
	//Gameplay/Video
    #define DO_X float4( 0.096 , 0.032 , 0.425 , 0.100 ) //Pos A1 = XY Color & A2 = ZW Black 
    #define DO_Y float4( 0.582 , 0.500 , 0.191 , 0.032 ) //Pos A3 = XY Color & B1 = ZW Color
    #define DO_Z float4( 0.425 , 0.100 , 0.582 , 0.50 ) //Pos B2 = XY Black & B3 = ZW Color
	#define DO_W float4( 14.0, 30.0, 14.0, 30.0) //Tresh Hold for Color A & B and Color
	//Controls/???
    #define DP_X float4( 0.379 , 0.032 , 0.425 , 0.100 ) //Pos C1 = XY Color & C2 = ZW Black 
    #define DP_Y float4( 0.582 , 0.500 , 0.000 , 0.000 ) //Pos C3 = XY Color & D1 = ZW Color
    #define DP_Z float4( 0.000 , 0.000 , 0.000 , 0.000 ) //Pos D2 = XY Black & D3 = ZW Color
	#define DP_W float4( 14.0, 30.0, 1000.0, 1000.0) //Tresh Hold for Color C & D and Color
  ...
  //  #define SDU 1	
	...
	#define PEW 1 
	#define DAA 1
#elif (App == 0x8EF426A9 ) //RoboQuest
    //#define DS_Z 2            // Set View Mode
	#define DA_W 1            // Set Linerzation
    //#define DB_X 1            // Flip
	#define DA_X 0.025          // ZPD
	#define DF_Y 0.00           // Seperation
	#define DA_Y 400.0          // Near Plane Adjustment
    //#define DA_Z -0.125       // Linerzation Offset
    #define DS_Y 2            // Linerzation Offset Effects only distance if true
	#define DB_Z 0.05         // Auto Depth Protection
	#define DE_X 5            // ZPD Boundary 
    ...
    //Simple Menu Detection
    #define SMD 4 //Off 0 | 1 | 2 | 3 | 4 
    #define DW_X float4( 0.501 , 0.973 , 0.500 , 0.125)  //Pos A = XY Any & B = ZW Lock 
    #define DW_Y float2( 0.903 , 0.962 )                 //Pos C = XY 
    #define DW_Z float4( 30.0, 0.0, 15.0, 1000.0) //Menu Detection Type for A = X, B = Y, & C = Z. The Last Value is a Wild Card amount W is for X and Z. 

    #define DT_X float4( 0.344 , 0.185 , 0.500 , 0.100)  //Pos A = XY Any & B = ZW Lock 
    #define DT_Y float2( 0.903 , 0.962 )                 //Pos C = XY 
    #define DW_W float4( 30.0, 0.0, 15.0, 1000.0) //Menu Detection Type for A = X, B = Y, & C = Z. The Last Value is a Wild Card amount W is for X and Z. 

    #define DAA_X float4( 0.500 , 0.154 , 0.500 , 0.100)  //Pos A = XY Any & B = ZW Lock 
    #define DAA_Y float2( 0.903 , 0.962 )                 //Pos C = XY 
    #define DAA_Z float4( 30.0, 0.0, 15.0, 1000.0) //Menu Detection Type for A = X, B = Y, & C = Z. The Last Value is a Wild Card amount W is for X and Z. 

    #define DBB_X float4( 0.525 , 0.869 , 0.500 , 0.125)  //Pos A = XY Any & B = ZW Lock 
    #define DBB_Y float2( 0.903 , 0.962 )                 //Pos C = XY 
    #define DBB_Z float4( 30.0, 0.0, 15.0, 1000.0) //Menu Detection Type for A = X, B = Y, & C = Z. The Last Value is a Wild Card amount W is for X and Z. 
  ...
  //  #define SDU 1	
	...
	//#define DG_W -0.25        // Shift Boundary Out of screen 0.5 and or In screen -0.5
	#define OIL 1           // Set How many Levels We use for RE_Fix 0 | 1 | 2 | 3 if 1 then it's float2(0,0) for OIF and DI_W
    #define OIF float2(0.5,0.025)// Fix enables if Value is > 0.0 
	#define DI_W float2(0.5,7.5) // Like Shift Boundary DG_W But 0 to inf
	//#define WND -0.5         //Weapon Near Pushes depth in and adjust perspective to match.
    //#define DG_Z 0.025        // Min Weapon Hands That are apart of world with Auto and Trim
    //#define DS_X float3(0.025,0,1)// Min Weapon bit only triggers when a OIL Level is set and set here on .y
    //#define DE_W 0.50        // Auto
  ...
  //  #define SDU 1	
	...
	#define DAA 1
	#define DSW 1
#elif (App == 0xB94D560A ) //GhostRunner 2
    //#define DS_Z 2            // Set View Mode
	#define DA_W 1            // Set Linerzation
    //#define DB_X 1            // Flip
  ...
  //  #define SDU 1	
	...
	#define DF_X float2(0.25,0.35)  // ZPD Weapon Boundarys Level 1 and Level 2
	//#define DJ_W 0.1	      // Weapon Depth Limit Location 1
	//#define DS_W 1.0	      // Weapon Depth Limit Location 2
	
  //Simple Menu Detection
    #define SMD 4 //Off 0 | 1 | 2 | 3 | 4 
    #define DW_X float4( 0.219, 0.193 , 0.157 , 0.8595)  //Pos A = XY Any & B = ZW Lock 
    #define DW_Y float2( 0.78 , 0.193 )                 //Pos C = XY 
    #define DW_Z float4( 28.0, 22.0, 28.0, 29.0) //Menu Detection Type for A = X, B = Y, & C = Z. The Last Value is a Wild Card amount W is for X and Z. 
    
    #define DT_X float4( 0.225, 0.115 , 0.500 , 0.150)  //Pos A = XY Any & B = ZW Lock 
    #define DT_Y float2( 0.769 , 0.115 )                 //Pos C = XY 
    #define DW_W float4( 30.0, 0.0, 30.0, 1000.0) //Menu Detection Type for A = X, B = Y, & C = Z. The Last Value is a Wild Card amount W is for X and Z. 

    #define DAA_X float4( 0.219, 0.192 , 0.157 , 0.8595)  //Pos A = XY Any & B = ZW Lock 
    #define DAA_Y float2( 0.773 , 0.192 )                 //Pos C = XY 
    #define DAA_Z float4( 28.0, 22.0, 28.0, 1000.0) //Menu Detection Type for A = X, B = Y, & C = Z. The Last Value is a Wild Card amount W is for X and Z. 

    #define DBB_X float4( 0.219, 0.1135 , 0.500 , 0.150)  //Pos A = XY Any & B = ZW Lock 
    #define DBB_Y float2( 0.776 , 0.1135 )                 //Pos C = XY 
    #define DBB_Z float4( 28.0, 0.0, 28.0, 29.0) //Menu Detection Type for A = X, B = Y, & C = Z. The Last Value is a Wild Card amount W is for X and Z. 
  ...
  //  #define SDU 1	
    ...
    //Lock Out Menu Detectors  
	#define LMD 1 //Off 0 | 1
    #define DCC_X float4( 0.500 , 0.963 , 0.398 , 0.925)  //Pos A = XY Any & B = ZW Lock 
    #define DCC_Y float2( 0.500 , 0.917 )                 //Pos C = XY 
    #define DCC_Z float4( 30.0, 13.0, 30.0, 1000.0) //Menu Detection Type for A = X, B = Y, & C = Z. The Last Value is a Wild Card amount W is for X and Z. 

    //Menu Detection Templates -  Needs a Specail Shader to adjust
    #define MDD 1 //Set Menu Detection & Direction      //Off | 1 | 2 | 3 | 4      
    #define DN_X float4( 0.581 , 0.136 , 0.591 , 0.293) //Pos A = XY Any & B = ZW Lock 
    #define DN_Y float4( 0.836 , 0.248 ,  0.0, 0.0)     //Pos C = XY Any & D = ZW Match
    #define DN_Z float4( 0.0, 0.0,  0.0, 0.0)           //Pos E = XY Match & F = ZW Match
	#define DN_W float4( 1.0, 0.0 , 0.0, 0.0 )          //Size = Menu [ABC] D E F
    #define DJ_Y float4( 25.0, 0.0, 25.0, 1000.0)       //Menu Detection Type for A = X, B = Y, & C = Z. The Last Value is a Wild Card amount W is for X and Z. 
    #define DJ_Z float4( 1000., 1000., 1000,0.0)           //Set Match Tresh 

	//Simple Menu Detection
    #define SMD 4 //Off 0 | 1 | 2 | 3 | 4 
    #define DW_X float4( 0.372 , 0.1445 , 0.841 , 0.500)  //Pos A = XY Any & B = ZW Lock 
    #define DW_Y float2( 0.631 , 0.1445 )                 //Pos C = XY 
    #define DW_Z float4( 27.0, 30.0, 27.0, 1000.0) //Menu Detection Type for A = X, B = Y, & C = Z. The Last Value is a Wild Card amount W is for X and Z. 

    //Multi Menu Detection                    //4 Blocks total and 2 sub blocks per one block.
    #define MMD 2 //Set Multi Menu Detection  // Off  0 | 1 | 2 | 3 | 4
    #define MMS 1 //Set Multi Menu Selection  // Off  0 | 1 | 2 | 3 | 4                       //0-2 check to 28-30 check Depnding on the option MML
    #define MML -2 //Set Multi Menu Leniency   // -4 |-3 |-2 |-1 | Off | 1 | 2 | 3 | 4         //Negitive [0 & 30] No Leniency | Zero [0-1 & 29-30] Some Leniency | Positive [0-2 & 28-30] Most Leniency    
	//Block One
    #define DO_X float4( 0.372 , 0.1445 , 0.491 , 0.222 ) //Pos A1 = XY Color & A2 = ZW Black 
    #define DO_Y float4( 0.631 , 0.1445 , 0.372 , 0.1445 ) //Pos A3 = XY Color & B1 = ZW Color
    #define DO_Z float4( 0.408 , 0.220 , 0.631 , 0.1445 ) //Pos B2 = XY Black & B3 = ZW Color
	#define DO_W float4( 27.0, 27.0, 27.0, 27.0) //Tresh Hold for Color A & B and Color

    #define DP_X float4( 0.372 , 0.1445 , 0.503 , 0.500 ) //Pos C1 = XY Color & C2 = ZW Black 
    #define DP_Y float4( 0.631 , 0.1445 , 0.335 , 0.1445 ) //Pos C3 = XY Color & D1 = ZW Color
    #define DP_Z float4( 0.654 , 0.881 , 0.661 , 0.1445 ) //Pos D2 = XY Black & D3 = ZW Color
	#define DP_W float4( 27.0, 27.0, 27.0, 27.0) //Tresh Hold for Color C & D and Color
    //Block Two
	#define DQ_X float4( 0.369 , 0.1445 , 0.426 , 0.250 ) //Pos C1 = XY Color & C2 = ZW Black 
    #define DQ_Y float4( 0.637 , 0.1445 , 0.369 , 0.1445 ) //Pos C3 = XY Color & D1 = ZW Color
    #define DQ_Z float4( 0.835 , 0.495 , 0.637 , 0.1445 ) //Pos D2 = XY Black & D3 = ZW Color
	#define DQ_W float4( 27.0, 27.0, 27.0, 27.0) //Tresh Hold for Color A1 & A3 and Color
	...
	#define PEW 1 
	#define DAA 1
#elif (App == 0x6DE081CF ) //Alan Wake 2
    //#define DS_Z 3            // Set View Mode
	//#define DA_W 1            // Set Linerzation
    //#define DB_X 1            // Flip
	#define DA_X 0.055 //0.08//0.025         // ZPD
	#define DF_Y 0.00          // Seperation
	#define DA_Y 12.5//7.5//27.5          // Near Plane Adjustment
    //#define DA_Z -0.125       // Linerzation Offset
    #define DS_Y 2            // Linerzation Offset Effects only distance if true
	#define DB_Z 0.025         // Auto Depth Protection
	#define DE_X 2            // ZPD Boundary 
	...
	#define OIL 3           // Set How many Levels We use for RE_Fix 0 | 1 | 2 | 3 if 1 then it's float2(0,0) for OIF and DI_W
    #define OIF float4(0.625,0.5,0.375,0.25)// Fix enables if Value is > 0.0 
	#define DI_W float4(0.25,0.5,1.000,3.00) // Like Shift Boundary DG_W But 0 to inf
	#define WND 0.5         //Weapon Near Pushes depth in and adjust perspective to match.
    //#define DG_Z 0.025        // Min Weapon Hands That are apart of world with Auto and Trim
    //#define DS_X float3(0.025,0,1)// Min Weapon bit only triggers when a OIL Level is set and set here on .y
    //#define DE_W 0.50        // Auto
  ...
  //  #define SDU 1	
	...
	#define DG_W 0.5        // Shift Boundary Out of screen 0.5 and or In screen -0.5
	//#define OIL 2           // Set How many Levels We use for RE_Fix 0 | 1 | 2 | 3 if 1 then it's float2(0,0) for OIF and DI_W
    //#define OIF 0.75 // Fix enables if Value is > 0.0 
	//#define DI_W 1.0 // Like Shift Boundary DG_W But 0 to inf
	//#define FTM 0             // Fast Trigger Mode If this enabled then Level 1 and > switches instantly.
	#define DG_Z 0.025        // Min Weapon Hands That are apart of world with Auto and Trim
    //#define DS_X float3(0.025,0,1)// Min Weapon bit only triggers when a OIL Level is set and set here on .y
    ...
    #define DI_Z 0.025        // Trim
	#define WND 1.0         //Weapon Near Pushes depth in and adjust perspective to match.
    #define DF_W float4(0.0001,0.001,0.0,0.0125)// Edge & Scale
  ...
  //Simple Menu Detection
    #define SMD 3 //Off 0 | 1 | 2 | 3 | 4 
    #define DW_X float4( 0.361, 0.464 , 0.512 , 0.500)  //Pos A = XY Any & B = ZW Lock 
    #define DW_Y float2( 0.637 , 0.668 )                 //Pos C = XY 
    #define DW_Z float4( 30.0, 22.0, 30.0, 1000.0) //Menu Detection Type for A = X, B = Y, & C = Z. The Last Value is a Wild Card amount W is for X and Z. 
    
    #define DT_X float4( 0.500, 0.875 , 0.500 , 0.12)  //Pos A = XY Any & B = ZW Lock 
    #define DT_Y float2( 0.936 , 0.904 )                 //Pos C = XY 
    #define DW_W float4( 0.0, 0.0, 30.0, 1000.0) //Menu Detection Type for A = X, B = Y, & C = Z. The Last Value is a Wild Card amount W is for X and Z. 

    #define DAA_X float4( 0.433, 0.910 , 0.318 , 0.399)  //Pos A = XY Any & B = ZW Lock 
    #define DAA_Y float2( 0.545 , 0.911 )                 //Pos C = XY 
    #define DAA_Z float4( 30.0, 0.0, 30.0, 27.0) //Menu Detection Type for A = X, B = Y, & C = Z. The Last Value is a Wild Card amount W is for X and Z. 
  ...
  //  #define DBB_Y float2( 0.500 , 0.004 )                 //Pos C = XY 
   // #define DBB_Z float4( 1000.0, 1000.0, 1000.0, 1000.0) //Menu Detection Type for A = X, B = Y, & C = Z. The Last Value is a Wild Card amount W is for X and Z. 
	...
	#define DG_W -0.25        // Shift Boundary Out of screen 0.5 and or In screen -0.5
	#define OIL 3           // Set How many Levels We use for RE_Fix 0 | 1 | 2 | 3 if 1 then it's float2(0,0) for OIF and DI_W
    #define OIF float4(0.6,0.300,0.175,0.075)// Fix enables if Value is > 0.0 
	#define DI_W float4(0.25,0.75,1.5,3.0) // Like Shift Boundary DG_W But 0 to inf
	//#define OIL 2           // Set How many Levels We use for RE_Fix 0 | 1 | 2 | 3 if 1 then it's float2(0,0) for OIF and DI_W
    //#define OIF 0.75 // Fix enables if Value is > 0.0 
	//#define DI_W 1.0 // Like Shift Boundary DG_W But 0 to inf
    ...
    #define MDD 1 //Set Menu Detection & Direction        //Off | 1 Left | 2 Right | 3 Bottom | 4 Top 
	#define COD 0 //Enable or Disable Cut-Off Direction   //Off | 1 Top & Bottom for One & Two or Left and Right for Three & Four 
    #define DN_X float4( 0.500, 0.682 , 0.500 , 0.931)   //Pos A = XY Any & B = ZW Lock 
    #define DN_Y float4( 0.252 , 0.768 ,  0.0, 0.0)       //Pos C = XY Any & D = ZW Match
    #define DN_Z float4( 0.0, 0.0,  0.0, 0.0)             //Pos E = XY Match & F = ZW Match
	#define DN_W float4( 1.0, 0.0 , 0.0, 0.0 )            //Size = Menu [ABC] D E F
    #define DJ_Y float4( 15.0, 0.0, 30.0, 1000.0)        //Menu Detection Type for A = X, B = Y, & C = Z. The Last Value is a Wild Card amount W is for X and Z. 
    #define DJ_Z float4( 1000., 1000., 1000, 0.0)         //Set Match Tresh and IU Y Elevation or Cut-Off -1-1

  //Text Menu Detection
    #define TMD 1 //Off 0 | 1Normal | 2 Inverse | 3 Mirror
    #define DZ_X float4( 0.483 , 0.665 , 0.500 , 0.936)  //Pos A = XY Any & B = ZW Lock 
    #define DZ_Y float2( 0.579 , 0.953 )                 //Pos C = XY 
    #define DZ_Z float4( 30.0, 0.0, 27.0, 1000.0) //Text Detection Type for A = X, B = Y, & C = Z. The Last Value is a Wild Card amount W is for X and Z. 
    #define DZ_W float4( 1.0, 1.000, 0.0, 0.692)             //Text Distance from screen, Text Cutoff point, and adjustable XY Masking.

 //Simple Menu Detection
    #define SMD 4 //Off 0 | 1 | 2 | 3 | 4 
    #define DW_X float4( 0.055 , 0.100 , 0.559 , 0.954)  //Pos A = XY Any & B = ZW Lock 
    #define DW_Y float2( 0.925 , 0.900 )                 //Pos C = XY 
    #define DW_Z float4( 0.0, 28.0, 0.0, 1000.0) //Menu Detection Type for A = X, B = Y, & C = Z. The Last Value is a Wild Card amount W is for X and Z. 
    
    #define DT_X float4( 0.483 , 0.665 , 0.500 , 0.936)  //Pos A = XY Any & B = ZW Lock 
    #define DT_Y float2( 0.579 , 0.953 )                 //Pos C = XY 
    #define DW_W float4( 30.0, 0.0, 27.0, 1000.0) //Menu Detection Type for A = X, B = Y, & C = Z. The Last Value is a Wild Card amount W is for X and Z. 

    #define DAA_X float4( 0.517, 0.515 , 0.500 , 0.581)  //Pos A = XY Any & B = ZW Lock 
    #define DAA_Y float2( 0.425 , 0.616 )                 //Pos C = XY 
    #define DAA_Z float4( 15.0, 0.0, 30.0, 1000.0) //Menu Detection Type for A = X, B = Y, & C = Z. The Last Value is a Wild Card amount W is for X and Z. 

    #define DBB_X float4( 0.250 , 0.250 , 0.955 , 0.928)  //Pos A = XY Any & B = ZW Lock 
    #define DBB_Y float2( 0.250 , 0.750 )                 //Pos C = XY 
    #define DBB_Z float4( 30.0, 0.0, 30.0, 1000.0) //Menu Detection Type for A = X, B = Y, & C = Z. The Last Value is a Wild Card amount W is for X and Z. 
	...
	#define PEW 1
	#define DAA 1
#elif (App == 0xB46249F0 ) //Jusant
    //#define DS_Z 2            // Set View Mode
	#define DA_W 1            // Set Linerzation
    //#define DB_X 1            // Flip
	#define DA_X 0.025         // ZPD
	#define DF_Y 0.000         // Seperation
	#define DA_Y 100         // Near Plane Adjustment
    //#define DA_Z -0.50 //0.0004      // Linerzation Offset
    //#define DS_Y 2            // Linerzation Offset Effects only distance if true
	#define DB_Z 0.05         // Auto Depth Protection
	#define DE_X 3            // ZPD Boundary 
	...
	//#define DG_W -0.25        // Shift Boundary Out of screen 0.5 and or In screen -0.5
	#define OIL 3           // Set How many Levels We use for RE_Fix 0 | 1 | 2 | 3 if 1 then it's float2(0,0) for OIF and DI_W
    #define OIF float4(0.5,0.375,0.25,0.15)// Fix enables if Value is > 0.0 
	#define DI_W float4(0.5,1.25,2.5,5.0) // Like Shift Boundary DG_W But 0 to inf
	//#define OIL 2           // Set How many Levels We use for RE_Fix 0 | 1 | 2 | 3 if 1 then it's float2(0,0) for OIF and DI_W
    //#define OIF 0.75 // Fix enables if Value is > 0.0 
	//#define DI_W 1.0 // Like Shift Boundary DG_W But 0 to inf
 ...
 //Menu Detection Templates -  Needs a Specail Shader to adjust
    #define MDD 1 //Set Menu Detection & Direction      //Off | 1 | 2 | 3 | 4      
    #define DN_X float4( 0.066 , 0.141 , 0.245 , 0.450) //Pos A = XY Any & B = ZW Lock 
    #define DN_Y float4( 0.215 , 0.083 , 0.800 , 0.500)     //Pos C = XY Any & D = ZW Match
    #define DN_Z float4( 0.0, 0.0,  0.0, 0.0)           //Pos E = XY Match & F = ZW Match
	#define DN_W float4( 0.225, 1.0 , 0.0, 0.0 )          //Size = Menu [ABC] D E F
    #define DJ_Y float4( 30.0, 3.0, 30.0, 1000.0)       //Menu Detection Type for A = X, B = Y, & C = Z. The Last Value is a Wild Card amount W is for X and Z. 
    #define DJ_Z float4( 3.0, 1000., 1000.,0.0)           //Set Match Tresh 
	...
	#define DAA 1
	#define DSW 1
#elif (App == 0xA1528B5B ) //Moria 
    //#define DS_Z 2            // Set View Mode
	#define DA_W 1            // Set Linerzation
    //#define DB_X 1            // Flip
	#define DA_X 0.025         // ZPD
	#define DF_Y 0.000         // Seperation
	#define DA_Y 75         // Near Plane Adjustment
    //#define DA_Z -0.50 //0.0004      // Linerzation Offset
    //#define DS_Y 2            // Linerzation Offset Effects only distance if true
	#define DB_Z 0.10         // Auto Depth Protection
	#define DE_X 3            // ZPD Boundary 
	#define DE_Y 0.75        // Set ZPD Boundary Level Zero 
	#define DE_Z 0.3        // Speed that Boundary is Enforced
	//#define AFD 1           // Alternate Frame Detection - May be phased out
 ...
 //Menu Detection Templates -  Needs a Specail Shader to adjust
    #define MDD 1 //Set Menu Detection & Direction      //Off | 1 | 2 | 3 | 4      
    #define DN_X float4( 0.127 , 0.231 , 0.500 , 0.050) //Pos A = XY Any & B = ZW Lock 
    #define DN_Y float4( 0.316 , 0.223 , 0.000 , 0.000)     //Pos C = XY Any & D = ZW Match
    #define DN_Z float4( 0.0, 0.0,  0.0, 0.0)           //Pos E = XY Match & F = ZW Match
	#define DN_W float4( 0.471, 0.0 , 0.0, 0.0 )          //Size = Menu [ABC] D E F
    #define DJ_Y float4( 26.0, 0.0, 26.0, 24.0)       //Menu Detection Type for A = X, B = Y, & C = Z. The Last Value is a Wild Card amount W is for X and Z. 
    #define DJ_Z float4( 3.0, 1000., 1000, 0.08)           //Set Match Tresh and Last one elevation 

    //Simple Menu Detection
    #define SMD 4 //Off 0 | 1 | 2 | 3 | 4 
    #define DW_X float4( 0.596 , 0.082 , 0.068 , 0.9615)  //Pos A = XY Any & B = ZW Lock 
    #define DW_Y float2( 0.634 , 0.068)                 //Pos C = XY 
    #define DW_Z float4( 22.0, 15.0, 22.0, 1000.0) //Menu Detection Type for A = X, B = Y, & C = Z. The Last Value is a Wild Card amount W is for X and Z. 

    #define DT_X float4( 0.903 , 0.0785 , 0.068 , 0.9615)  //Pos A = XY Any & B = ZW Lock 
    #define DT_Y float2( 0.863 , 0.076 )                 //Pos C = XY 
    #define DW_W float4( 22.0, 15.0, 22.0, 1000.0) //Menu Detection Type for A = X, B = Y, & C = Z. The Last Value is a Wild Card amount W is for X and Z. 
    
    #define DAA_X float4( 0.596 , 0.082 , 0.1425, 0.932)  //Pos A = XY Any & B = ZW Lock 
    #define DAA_Y float2( 0.9035 , 0.0785 )                 //Pos C = XY 
    #define DAA_Z float4( 22.0, 26.0, 22.0, 25.0) //Menu Detection Type for A = X, B = Y, & C = Z. The Last Value is a Wild Card amount W is for X and Z. 
	
    #define DBB_X float4( 0.564, 0.073 , 0.050 , 0.962)  //Pos A = XY Any & B = ZW Lock 
    #define DBB_Y float2( 0.935, 0.073 )                 //Pos C = XY 
    #define DBB_Z float4( 30.0, 30.0, 30.0, 1000.0) //Menu Detection Type for A = X, B = Y, & C = Z. The Last Value is a Wild Card amount W is for X and Z. 

    //Stencil UI & Detection
    #define SUI 1 //Off 0 | 1 Square | 2 Circle "Future Update"
    #define SSA 1 //Off 0 | 1 Square | 2 Circle
    #define DDD_X float4( 0.1425, 0.932 , 0.964 , 0.065)  //Pos A = XY Any & B = ZW Lock 
    #define DDD_Y float4( 0.958 , 0.783 , 0.5 , 0.929)      //Pos C = XY Any & UI Postion 
    #define DDD_Z float4( 26.0, 25.0, 22.0, 25.0) //Detection Value 0-30 for A = X, B = Y, & C = Z. The Last Value is a Wild Card amount W is for X and Z. 
    #define DDD_W float4( 0.325, 0.450, 1.0, 0.0625)             //X is Hoz Size and Y is Vert Size, Z is Inversion amount and W is Depth set elevation with 0.0-0.5 is Auto.
	...
	#define DA_X 0.050         // ZPD
	#define DF_Y 0.000         // Seperation
	#define DA_Y 37.0         // Near Plane Adjustment
    //#define DA_Z -0.50 //0.0004      // Linerzation Offset
    //#define DS_Y 2            // Linerzation Offset Effects only distance if true
	#define DB_Z 0.025         // Auto Depth Protection
	#define DE_X 5            // ZPD Boundary 
	...
	#define OIL 1           // Set How many Levels We use for RE_Fix 0 | 1 | 2 | 3 if 1 then it's float2(0,0) for OIF and DI_W
    #define OIF float2(0.5,0.375)// Fix enables if Value is > 0.0 
	#define DI_W float2(1.0,1.5) // Like Shift Boundary DG_W But 0 to inf
	//#define OIL 2           // Set How many Levels We use for RE_Fix 0 | 1 | 2 | 3 if 1 then it's float2(0,0) for OIF and DI_W
    //#define OIF 0.75 // Fix enables if Value is > 0.0 
	//#define DI_W 1.0 // Like Shift Boundary DG_W But 0 to inf
	//#define FTM 0             // Fast Trigger Mode If this enabled then Level 1 and > switches instantly.
	...
	#define AWZ 1.0          // Anti-Weapon Hand Z-Fighting -1 is shifted to the right a little bit and 1 is center
	//#define WBS 0             //Weapon Boundary Switch 0 | 1
	#define DF_X float2(0.1,0.2)  // ZPD Weapon Boundarys Level 1 and Level 2
	#define DJ_W 1.0	      // Weapon Depth Limit Location 1
	#define DS_W 1.5	      // Weapon Depth Limit Location 2
	//#define WFB 1.0                // ZPD Weapon Elevaton for 1 and 2 scales from [0 - 1]

	//Lock Out Menu Detectors  
	#define LMD 1 //Off 0 | 1
    #define DCC_X float4( 0.025, 0.817 , 0.025 , 0.925 )  //Pos A = XY Any & B = ZW Lock 
    #define DCC_Y float2( 0.039 , 0.9505 )                 //Pos C = XY 
    #define DCC_Z float4( 11.0, 0.0, 10.0, 1000.0) //Menu Detection Type for A = X, B = Y, & C = Z. The Last Value is a Wild Card amount W is for X and Z. 

	//Simple Menu Detection - english only
    #define SMD 4 //Off 0 | 1 | 2 | 3 | 4 
    //Paused
    #define DW_X float4( 0.369, 0.123 , 0.496 , 0.123)    //Pos A = XY Any & B = ZW Lock 
    #define DW_Y float2( 0.5015, 0.344 )                   //Pos C = XY 
    #define DW_Z float4( 12.0, 0.0, 0.0, 1000.0)   //Menu Detection Type for A = X, B = Y, & C = Z. The Last Value is a Wild Card amount W is for X and Z. 
    
    #define DT_X float4( 0.192 , 0.104 , 0.205 , 0.100)    //Pos A = XY Any & B = ZW Lock 
    #define DT_Y float2( 0.205 , 0.806 )                   //Pos C = XY 
    #define DW_W float4( 12.0, 0.0, 0.0, 1000.0)   //Menu Detection Type for A = X, B = Y, & C = Z. The Last Value is a Wild Card amount W is for X and Z. 
 
    #define DAA_X float4( 0.949, 0.022 , 0.498 , 0.259)    //Pos A = XY Any & B = ZW Lock 
    #define DAA_Y float2( 0.142 , 0.982 )                   //Pos C = XY 
    #define DAA_Z float4( 10.0, 0.0, 10.0, 1000.0)   //Menu Detection Type for A = X, B = Y, & C = Z. The Last Value is a Wild Card amount W is for X and Z. 

    #define DBB_X float4( 0.745, 0.129 , 0.500 , 0.255)    //Pos A = XY Any & B = ZW Lock 
    #define DBB_Y float2( 0.675 , 0.563 )                   //Pos C = XY 
    #define DBB_Z float4( 30.0, 0.0, 30.0, 1000.0)   //Menu Detection Type for A = X, B = Y, & C = Z. The Last Value is a Wild Card amount W is for X and Z. 
    ...
    //#define DM_Y 0     //HQ VRS 
	#define PEW 1
#elif (App == 0x6250A9E ) //Zombie Army Dead War 4
    //#define DS_Z 2            // Set View Mode
	//#define DA_W 1            // Set Linerzation
    //#define DB_X 1            // Flip
	#define DA_X 0.025         // ZPD
    ...
    #define SSA 2 //Off 0 | 1 Square | 2 Circle
    #define SNA 1 //Off 0 | 1 Trigger on Aim
    #define DDD_X float4( 0.197 , 0.386 , 0.500 , 0.975)  //Pos A = XY Any & B = ZW Lock 
    #define DDD_Y float4( 0.239 , 0.640 , 0.5 , 0.5)      //Pos C = XY Any & UI Postion 
    #define DDD_Z float4( 30.0, 0.0, 0.0, 1000.0) //Detection Value 0-30 for A = X, B = Y, & C = Z. The Last Value is a Wild Card amount W is for X and Z. 
    #define DDD_W float4( 0.4875, 0.0, 1.0, 0.0625)             //X is Hoz Size and Y is Vert Size, Z is Inversion amount and W is Depth set elevation with 0.0-0.5 is Auto.

	//Simple Menu Detection
    #define SMD 1 //Off 0 | 1 | 2 | 3 | 4 
    #define DW_X float4( 0.156 , 0.231 , 0.308 , 0.282)    //Pos A = XY Any & B = ZW Lock 
    #define DW_Y float2( 0.166 , 0.344 )                   //Pos C = XY 
    #define DW_Z float4( 30.0, 0.0, 9.0, 1000.0)   //Menu Detection Type for A = X, B = Y, & C = Z. The Last Value is a Wild Card amount W is for X and Z. 
    ...
    #define MMD 3 //Set Multi Menu Detection  // Off  0 | 1 | 2 | 3 | 4
    #define MMS 0 //Set Multi Menu Selection  // Off  0 | 1 | 2 | 3 | 4                       //0-2 check to 28-30 check Depnding on the option MML
    #define MML -3 //Set Multi Menu Leniency   // -4 |-3 |-2 |-1 | Off | 1 | 2 | 3 | 4         //Negitive [0 & 30] No Leniency | Zero [0-1 & 29-30] Some Leniency | Positive [0-2 & 28-30] Most Leniency    
	//Main Menu & Options Game
    #define DO_X float4( 0.071 , 0.919 , 0.917 , 0.070 ) //Pos A1 = XY Color & A2 = ZW Black 
    #define DO_Y float4( 0.303 , 0.911 , 0.052 , 0.140 ) //Pos A3 = XY Color & B1 = ZW Color
    #define DO_Z float4( 0.917 , 0.070 , 0.175 , 0.162 ) //Pos B2 = XY Black & B3 = ZW Color
	#define DO_W float4( 30.0, 30.0, 30.0, 30.0) //Tresh Hold for Color A & B and Color
	//Options Controls & Options Display 
    #define DP_X float4( 0.182 , 0.137 , 0.917 , 0.070 ) //Pos C1 = XY Color & C2 = ZW Black 
    #define DP_Y float4( 0.305 , 0.162 , 0.310 , 0.140 ) //Pos C3 = XY Color & D1 = ZW Color
    #define DP_Z float4( 0.917 , 0.070 , 0.434 , 0.162 ) //Pos D2 = XY Black & D3 = ZW Color
	#define DP_W float4( 30.0, 30.0, 30.0, 30.0) //Tresh Hold for Color C & D and Color
    //Options Audio & Options Accessiblity
	#define DQ_X float4( 0.440 , 0.138 , 0.917 , 0.070 ) //Pos C1 = XY Color & C2 = ZW Black 
    #define DQ_Y float4( 0.560 , 0.160 , 0.567 , 0.138 ) //Pos C3 = XY Color & D1 = ZW Color
    #define DQ_Z float4( 0.917 , 0.070 , 0.691 , 0.160 ) //Pos D2 = XY Black & D3 = ZW Color
	#define DQ_W float4( 30.0, 30.0, 30.0, 30.0) //Tresh Hold for Color A1 & A3 and Color
	//Horde Keyboard & Horde Controller
	#define DR_X float4( 0.063 , 0.925 , 0.917 , 0.070 ) //Pos G1 = XY Color & G2 = ZW Black 
    #define DR_Y float4( 0.923 , 0.142 , 0.063 , 0.935 ) //Pos G3 = XY Color & H1 = ZW Color
    #define DR_Z float4( 0.917 , 0.070 , 0.923 , 0.142 ) //Pos H2 = XY Black & H3 = ZW Color
	#define DR_W float4( 30.0, 30.0, 29.0, 30.0) //Tresh Hold for Color G & H and Color 
    //Horde Keyboard 2 &
	#define DU_X float4( 0.070 , 0.925 , 0.917 , 0.070 ) //Pos I1 = XY Color & I2 = ZW Black 
    #define DU_Y float4( 0.923 , 0.142 , 0.000 , 0.000 ) //Pos I3 = XY Color & J1 = ZW Color
    #define DU_Z float4( 0.000 , 0.000 , 0.000 , 0.000 ) //Pos J2 = XY Black & J3 = ZW Color
	#define DU_W float4( 30.0, 30.0, 1000.0, 1000.0) //Tresh Hold for Color I & J and Color
    ...
    #define FPS 0  // FPS Focus Settings 
    #define DK_X 3 //Trigger Type
    //#define DK_Y 0 //Eye Selection
    //#define WRP 3  //Weapon Reduction Power
    #define DK_Z 0 //World Reduction Power
	...
	#define DA_X 0.015         // ZPD
	//#define DF_Y 0.005         // Seperation
	#define DA_Y 150.0         // Near Plane Adjustment
    //#define DA_Z -0.50 //0.0004      // Linerzation Offset
    //#define DS_Y 2            // Linerzation Offset Effects only distance if true
	#define DB_Z 0.02         // Auto Depth Protection
	//#define DE_X 4            // ZPD Boundary 
	...
	//#define DI_W 1.0 // Like Shift Boundary DG_W But 0 to inf
	//#define FTM 0             // Fast Trigger Mode If this enabled then Level 1 and > switches instantly.
	#define DG_Z 0.040        // Min Weapon Hands That are apart of world with Auto and Trim
    //#define DS_X float3(0.025,0,1)// Min Weapon bit only triggers when a OIL Level is set and set here on .y
    #define DE_W 0.3        // Auto
    #define DI_Z 0.040        // Trim
	#define WND -1.0         //Weapon Near Pushes depth in and adjust perspective to match.
    #define DF_W float4(0.0001,0.002,0.0,0.002)// Edge & Scale
	...
	//Simple Menu Detection
    #define SMD 2 //Off 0 | 1 | 2 | 3 | 4 
    #define DW_X float4( 0.420, 0.925 , 0.500 , 0.050)    //Pos A = XY Any & B = ZW Lock 
    #define DW_Y float2( 0.580 , 0.960 )                   //Pos C = XY 
    #define DW_Z float4( 25.0, 0.0, 25.0, 24.0)   //Menu Detection Type for A = X, B = Y, & C = Z. The Last Value is a Wild Card amount W is for X and Z. 

    #define DT_X float4( 0.269 , 0.464 , 0.500 , 0.050)    //Pos A = XY Any & B = ZW Lock 
    #define DT_Y float2( 0.537 , 0.960 )                   //Pos C = XY 
    #define DW_W float4( 30.0, 0.0, 25.0, 24.0)   //Menu Detection Type for A = X, B = Y, & C = Z. The Last Value is a Wild Card amount W is for X and Z. 
	...
	#define PEW 1
	#define FOV 1
#elif (App == 0xFD3749FC )	//The Crew Motorfest 
    //#define DS_Z 2                 // Set View Mode
	#define DA_W 1                 // Set Linerzation
    //#define DB_X 1                 // Flip
	#define DA_X 0.025             // ZPD
	...
	#define DE_Z 0.375             // Speed that Boundary is Enforced
	//#define AFD 1                // Alternate Frame Detection - May be phased out
	#define DG_W 0.50             // Shift Boundary Out of screen 0.5 and or In screen -0.5
	#define OIL 1                // Set How many Levels We use for RE_Fix 0 | 1 | 2 | 3 if 1 then it's float2(0,0) for OIF and DI_W
    #define OIF float2(0.5,0.375)// Fix enables if Value is > 0.0 
	...
	//Lock Out Menu Detectors  
	#define LMD 1 //Off 0 | 1
    #define DCC_X float4( 0.090 , 0.920 , 0.213 , 0.803)  //Pos A = XY Any & B = ZW Lock 
    #define DCC_Y float2( 0.053 , 0.901 )                 //Pos C = XY 
    #define DCC_Z float4( 30.0, 2.0, 21.0, 1000.0) //Menu Detection Type for A = X, B = Y, & C = Z. The Last Value is a Wild Card amount W is for X and Z. 

	//Map WIP
    #define MDD 1 //Set Menu Detection & Direction        //Off | 1 Left | 2 Right | 3 Bottom | 4 Top 
	//#define COD 0 //Enable or Disable Cut-Off Direction   //Off | 1 Top & Bottom for One & Two or Left and Right for Three & Four 
	//Menu Bar
    #define DN_X float4( 0.022 , 0.349 , 0.025 , 0.406)   //Pos A = XY Any & B = ZW Lock 
    #define DN_Y float4( 0.947 , 0.070 ,  0.1565, 0.500)       //Pos C = XY Any & D = ZW Match
    #define DN_Z float4( 0.0, 0.0,  0.0, 0.0)             //Pos E = XY Match & F = ZW Match
	#define DN_W float4( 0.053, 0.157 , 0.0, 0.0 )            //Size = Menu [ABC] D E F
    #define DJ_Y float4( 21.0, 30.0, 2.0, 1000.0)        //Menu Detection Type for A = X, B = Y, & C = Z. The Last Value is a Wild Card amount W is for X and Z. 
    #define DJ_Z float4( 21.0, 1000., 1000, 0.0)         //Set Match Tresh and IU Y Elevation or Cut-Off -1-1    
	//Simple Menu Detection
    #define SMD 4 //Off 0 | 1 | 2 | 3 | 4 
    //PARTICIPATED
    #define DW_X float4( 0.306 , 0.255 , 0.500 , 0.069)    //Pos A = XY Any & B = ZW Lock 
    #define DW_Y float2( 0.693 , 0.255 )                   //Pos C = XY 
    #define DW_Z float4( 30.0, 0.0, 30.0, 1000.0)   //Menu Detection Type for A = X, B = Y, & C = Z. The Last Value is a Wild Card amount W is for X and Z. 
    // World Racing Menu
    #define DT_X float4( 0.701, 0.634 , 0.701 , 0.548)    //Pos A = XY Any & B = ZW Lock 
    #define DT_Y float2( 0.703 , 0.717 )                   //Pos C = XY 
    #define DW_W float4( 17.0, 0.0, 30.0, 1000.0 )   //Menu Detection Type for A = X, B = Y, & C = Z. The Last Value is a Wild Card amount W is for X and Z. 
	// Options
    #define DAA_X float4( 0.022, 0.349 , 0.025 , 0.406)    //Pos A = XY Any & B = ZW Lock 
    #define DAA_Y float2( 0.927 , 0.9248 )  //here                 //Pos C = XY 
    #define DAA_Z float4( 21.0, 30.0, 30.0, 1000.0)   //Menu Detection Type for A = X, B = Y, & C = Z. The Last Value is a Wild Card amount W is for X and Z. 
	//Activities
    #define DBB_X float4( 0.022, 0.349 , 0.025 , 0.406)    //Pos A = XY Any & B = ZW Lock 
    #define DBB_Y float2( 0.932 , 0.9247 )                   //Pos C = XY 
    #define DBB_Z float4( 21.0, 30.0, 30.0, 1000.0)   //Menu Detection Type for A = X, B = Y, & C = Z. The Last Value is a Wild Card amount W is for X and Z. 
	//Smooth Mode Setting
    #define SMS 3           //SM Toggle Separation
	#define DL_X 0.95       //SM Tune
	//#define DL_W 0.05       //SM Perspective
	...
	#define DA_X 0.0375//0.025 //0.050      // ZPD
	#define DF_Y 0.000         // Seperation
	#define DA_Y 41.5//62.5 //30.0        // Near Plane Adjustment
    #define DA_Z 0.00075      // Linerzation Offset
    #define DS_Y 1            // Linerzation Offset Effects only distance if true
	#define DB_Z 0.025         // Auto Depth Protection
	#define DE_X 5            // ZPD Boundary 
	...
	#define DG_W -0.25        // Shift Boundary Out of screen 0.5 and or In screen -0.5
	#define OIL 3           // Set How many Levels We use for RE_Fix 0 | 1 | 2 | 3 if 1 then it's float2(0,0) for OIF and DI_W
    #define OIF float4(0.50,0.350,0.250,0.125)// Fix enables if Value is > 0.0 
	#define DI_W float4(0.125,0.5,1.0,5.0) // Like Shift Boundary DG_W But 0 to inf
	//#define FTM 0             // Fast Trigger Mode If this enabled then Level 1 and > switches instantly.
    // DG_Z 0.045        // Min Weapon Hands That are apart of world with Auto and Trim
    //#define DS_X float3(0.025,0,1)// Min Weapon bit only triggers when a OIL Level is set and set here on .y
    //#define DE_W 0.250        // Auto
	...
	#define FOV 1
	#define PEW 1   
#elif (App == 0xF0E27D63 ) // State of Decay 2
    //#define DS_Z 2            // Set View Mode
	#define DA_W 1            // Set Linerzation
    //#define DB_X 1            // Flip
	#define DA_X 0.025         // ZPD
	//#define DF_Y 0.005         // Seperation
	#define DA_Y 41.5         // Near Plane Adjustment
    //#define DA_Z -0.50 //0.0004      // Linerzation Offset
    #define DS_Y 2            // Linerzation Offset Effects only distance if true
	#define DB_Z 0.1         // Auto Depth Protection
	#define DE_X 1           // ZPD Boundary 
	...
	#define OIL 3           // Set How many Levels We use for RE_Fix 0 | 1 | 2 | 3 if 1 then it's float2(0,0) for OIF and DI_W
    #define OIF float4(0.5,0.375,0.25,0.125)// Fix enables if Value is > 0.0 
	#define DI_W float4(0.25,1.0,2.5,5.0) // Like Shift Boundary DG_W But 0 to inf
	//#define OIL 2           // Set How many Levels We use for RE_Fix 0 | 1 | 2 | 3 if 1 then it's float2(0,0) for OIF and DI_W
    //#define OIF 0.75 // Fix enables if Value is > 0.0 
	//#define DI_W 1.0 // Like Shift Boundary DG_W But 0 to inf
	//#define FTM 0             // Fast Trigger Mode If this enabled then Level 1 and > switches instantly.
    ...
    #define DEE_W float4( 0.364, 0.160,0.95, 0.0)             //X is Hoz Size and Y is Vert Size, Z is Inversion amount and W is Depth set elevation with 0.0-0.5 > is Auto.
  ...
  //  #define DAA_Z float4( 27.0, 1.0, 2.0, 1000.0)   //Menu Detection Type for A = X, B = Y, & C = Z. The Last Value is a Wild Card amount W is for X and Z. 
	...
	#define FOV 1
	#define NDW 1
#elif (App == 0xDE4C92BB ) //Halo Infinite New
    //#define DS_Z 2            // Set View Mode
	#define DA_W 1            // Set Linerzation
    //#define DB_X 1            // Flip
	#define DA_X 0.025          // ZPD
	...
	#define OIL 3           // Set How many Levels We use for RE_Fix 0 | 1 | 2 | 3 if 1 then it's float2(0,0) for OIF and DI_W
    #define OIF float4(0.5,0.375,0.25,0.125)// Fix enables if Value is > 0.0 
	#define DI_W float4(0.25,1.0,2.0,3.75) // Like Shift Boundary DG_W But 0 to inf
	#define WND 1.0         //Weapon Near Pushes depth in and adjust perspective to match.
    //#define DG_Z 0.025        // Min Weapon Hands That are apart of world with Auto and Trim
    //#define DS_X float3(0.025,0,1)// Min Weapon bit only triggers when a OIL Level is set and set here on .y
    //#define DE_W 0.50        // Auto
  ...
  //  #define SDU 1	
	...
	//Simple Menu Detection
    #define SMD 2 //Off 0 | 1 | 2 | 3 | 4 
    #define DW_X float4( 0.028 , 0.963 , 0.765 , 0.943)    //Pos A = XY Any & B = ZW Lock 
    #define DW_Y float2( 0.944 , 0.974 )                   //Pos C = XY 
    #define DW_Z float4( 30.0, 1.0, 30.0, 1000.0)   //Menu Detection Type for A = X, B = Y, & C = Z. The Last Value is a Wild Card amount W is for X and Z. 
    
    #define DT_X float4( 0.028 , 0.963 , 0.478 , 0.210)    //Pos A = XY Any & B = ZW Lock 
    #define DT_Y float2( 0.041 , 0.124 )                   //Pos C = XY 
    #define DW_W float4( 30.0, 30.0, 30.0, 1000.0)   //Menu Detection Type for A = X, B = Y, & C = Z. The Last Value is a Wild Card amount W is for X and Z. 

    #define DAA_X float4( 0.028 , 0.963 , 0.765 , 0.943)    //Pos A = XY Any & B = ZW Lock 
    #define DAA_Y float2( 0.055 , 0.061 )                   //Pos C = XY 
    #define DAA_Z float4( 30.0, 1.0, 22.0, 1000.0)   //Menu Detection Type for A = X, B = Y, & C = Z. The Last Value is a Wild Card amount W is for X and Z. 
	...
	#define AWZ -2.0          // Anti-Weapon Hand Z-Fighting -1 is shifted to the right a little bit and 1 is center
	//#define WBS 0             //Weapon Boundary Switch 0 | 1
	#define DF_X float2(-0.25,0.0)  // ZPD Weapon Boundarys Level 1 and Level 2
	...
	#define PEW 1 
	#define DAA 1
#elif (App == 0xBEA3E9D1 ) //Teardown DX12
    //#define DS_Z 2            // Set View Mode
	...
	#define OIL 3           // Set How many Levels We use for RE_Fix 0 | 1 | 2 | 3 if 1 then it's float2(0,0) for OIF and DI_W
    #define OIF float4(0.5,0.375,0.25,0.125)// Fix enables if Value is > 0.0 
	#define DI_W float4(1.0,1.5,2.5,3.75) // Like Shift Boundary DG_W But 0 to inf
    #define CWH 2             //ZPD Weapon Hand Consideration For Masking  0 is Off | 1 Is Full | 2 Half Right screen Mask And Not to be used with Weapon Profiles.
    #define WBA 1.5           //ZPD Weapon Boundary Alt Adjust power for CWH 
	//#define FTM 0             // Fast Trigger Mode If this enabled then Level 1 and > switches instantly.
	#define WND 1.0         //Weapon Near Pushes depth in and adjust perspective to match.
    #define DG_Z 0.0125        // Min Weapon Hands That are apart of world with Auto and Trim
    //#define DS_X float3(0.025,0,1)// Min Weapon bit only triggers when a OIL Level is set and set here on .y
	...
	//Simple Menu Detection
    #define SMD 1 //Off 0 | 1 | 2 | 3 | 4 
    #define DW_X float4( 0.108 , 0.095 , 0.050 , 0.850)    //Pos A = XY Any & B = ZW Lock 
    #define DW_Y float2( 0.392 , 0.306 )                   //Pos C = XY 
    #define DW_Z float4( 30.0, 0.0, 30.0, 1000.0)   //Menu Detection Type for A = X, B = Y, & C = Z. The Last Value is a Wild Card amount W is for X and Z. 
	...
	#define OIL 3           // Set How many Levels We use for RE_Fix 0 | 1 | 2 | 3 if 1 then it's float2(0,0) for OIF and DI_W
    #define OIF float4(0.5,0.375,0.25,0.125)// Fix enables if Value is > 0.0 
	#define DI_W float4(0.5,2.5,3.75,5.0) // Like Shift Boundary DG_W But 0 to inf
	#define WND 1.0         //Weapon Near Pushes depth in and adjust perspective to match.
    //#define DG_Z 0.025        // Min Weapon Hands That are apart of world with Auto and Trim
    //#define DS_X float3(0.025,0,1)// Min Weapon bit only triggers when a OIL Level is set and set here on .y
    //#define DE_W 0.50        // Auto
  ...
  //  #define SDU 1	
    ...
    #define SSA 1 //Off 0 | 1 Square | 2 Circle
    //#define SNA 1 //Off 0 | 1 Trigger on Aim
    #define DDD_X float4( 0.080 , 0.162 , 0.92 , 0.162)  //Pos A = XY Any & B = ZW Lock 
    #define DDD_Y float4( 0.920 , 0.839 , 0.5 , 0.5)      //Pos C = XY Any & UI Postion 
    #define DDD_Z float4( 25.0, 25.0, 25.0, 1000.0) //Detection Value 0-30 for A = X, B = Y, & C = Z. The Last Value is a Wild Card amount W is for X and Z. 
    #define DDD_W float4( 0.075, 0.105, 1.0, 0.75)             //X is Hoz Size and Y is Vert Size, Z is Inversion amount and W is Depth set elevation with 0.0-0.5 > is Auto.

    #define SSB 1 //Off 0 | 1 Square | 2 Circle
    //#define SNB 1 //Off 0 | 1 Trigger on Aim
    #define DEE_X float4( 0.308 , 0.155 , 0.690 , 0.155)  //Pos A = XY Any & B = ZW Lock 
   ...
   //Settings
    #define MDD 1 //Set Menu Detection & Direction        //Off | 1 Left | 2 Right | 3 Bottom | 4 Top 
	#define COD 0 //Enable or Disable Cut-Off Direction   //Off | 1 Top & Bottom for One & Two or Left and Right for Three & Four 
    #define DN_X float4( 0.219 , 0.500 , 0.187 , 0.886)   //Pos A = XY Any & B = ZW Lock 
    #define DN_Y float4( 0.219 , 0.915 ,  0.975, 0.925)       //Pos C = XY Any & D = ZW Match
    #define DN_Z float4( 0.0, 0.0,  0.0, 0.0)             //Pos E = XY Match & F = ZW Match
	#define DN_W float4( 0.25, 1.0 , 0.0, 0.0 )            //Size = Menu [ABC] D E F
    #define DJ_Y float4( 18.0, 30.0, 18.0, 1000.0)        //Menu Detection Type for A = X, B = Y, & C = Z. The Last Value is a Wild Card amount W is for X and Z. 
    #define DJ_Z float4( 29., 1000., 1000, 0.0)         //Set Match Tresh and IU Y Elevation or Cut-Off -1-1

  	//Simple Menu Detection
    #define SMD 4 //Off 0 | 1 | 2 | 3 | 4 
    //Options
    #define DW_X float4( 0.069 , 0.097 , 0.929 , 0.097)    //Pos A = XY Any & B = ZW Lock 
    #define DW_Y float2( 0.929 , 0.903 )                   //Pos C = XY 
    #define DW_Z float4( 29.0, 29.0, 29.0, 1000.0)   //Menu Detection Type for A = X, B = Y, & C = Z. The Last Value is a Wild Card amount W is for X and Z. 
    //PalBox
    #define DT_X float4( 0.881, 0.866 , 0.596 , 0.140)    //Pos A = XY Any & B = ZW Lock 
    #define DT_Y float2( 0.339 , 0.633 )                   //Pos C = XY 
    #define DW_W float4( 29.0, 30.0, 28.0, 1000.0)   //Menu Detection Type for A = X, B = Y, & C = Z. The Last Value is a Wild Card amount W is for X and Z. 
	//Map
    #define DAA_X float4( 0.063 , 0.094 , 0.936 , 0.094)    //Pos A = XY Any & B = ZW Lock 
    #define DAA_Y float2( 0.936 , 0.905 )                   //Pos C = XY 
    #define DAA_Z float4( 29.0, 29.0, 29.0, 1000.0)   //Menu Detection Type for A = X, B = Y, & C = Z. The Last Value is a Wild Card amount W is for X and Z. 
	//Return to Title
    #define DBB_X float4( 0.264, 0.296 , 0.736 , 0.296)    //Pos A = XY Any & B = ZW Lock 
    #define DBB_Y float2( 0.736 , 0.704 )                   //Pos C = XY 
    #define DBB_Z float4( 30.0, 30.0, 30.0, 1000.0)   //Menu Detection Type for A = X, B = Y, & C = Z. The Last Value is a Wild Card amount W is for X and Z. 

    //Build Menu Detection                    //4 Blocks total and 2 sub blocks per one block.
    #define MMD 4 //Set Multi Menu Detection  // Off  0 | 1 | 2 | 3 | 4
    #define MMS 4 //Set Multi Menu Selection  // Off  0 | 1 | 2 | 3 | 4                       //0-2 check to 28-30 check Depnding on the option MML
    #define MML -4 //Set Multi Menu Leniency   // -4 |-3 |-2 |-1 | Off | 1 | 2 | 3 | 4         //Negitive [0 & 30] No Leniency | Zero [0-1 & 29-30] Some Leniency | Positive [0-2 & 28-30] Most Leniency    

	//Infrastructure & Production
    #define DO_X float4( 0.227 , 0.073 , 0.500 , 0.108 ) //Pos A1 = XY Color & A2 = ZW Black 
    #define DO_Y float4( 0.783 , 0.572 , 0.227 , 0.073 ) //Pos A3 = XY Color & B1 = ZW Color
    #define DO_Z float4( 0.271 , 0.108 , 0.783 , 0.572 ) //Pos B2 = XY Black & B3 = ZW Color
	#define DO_W float4( 28.0, 28.0, 28.0, 28.0) //Tresh Hold for Color A & B and Color
	//Pal & Storage
    #define DP_X float4( 0.227 , 0.073 , 0.334 , 0.108 ) //Pos C1 = XY Color & C2 = ZW Black 
    #define DP_Y float4( 0.783 , 0.572 , 0.227 , 0.073 ) //Pos C3 = XY Color & D1 = ZW Color
    #define DP_Z float4( 0.388 , 0.108 , 0.783 , 0.57) //Pos D2 = XY Black & D3 = ZW Color
	#define DP_W float4( 28.0, 28.0, 28.0, 28.0) //Tresh Hold for Color C & D and Color
    //Food and Lighting
	#define DQ_X float4( 0.227 , 0.073 , 0.446 , 0.108 ) //Pos C1 = XY Color & C2 = ZW Black 
    #define DQ_Y float4( 0.783 , 0.572 , 0.227 , 0.073 ) //Pos C3 = XY Color & D1 = ZW Color
    #define DQ_Z float4( 0.563 , 0.108 , 0.783 , 0.572 ) //Pos D2 = XY Black & D3 = ZW Color
	#define DQ_W float4( 28.0, 28.0, 28.0, 28.0) //Tresh Hold for Color A1 & A3 and Color
	//Fondations & Defence
	#define DR_X float4( 0.227 , 0.073 , 0.626 , 0.108 ) //Pos G1 = XY Color & G2 = ZW Black 
    #define DR_Y float4( 0.783 , 0.572 , 0.227 , 0.073 ) //Pos G3 = XY Color & H1 = ZW Color
    #define DR_Z float4( 0.684 , 0.108 , 0.783 , 0.572 ) //Pos H2 = XY Black & H3 = ZW Color
	#define DR_W float4( 28.0, 28.0, 28.0, 28.0) //Tresh Hold for Color G & H and Color 
    //Other and Furniture
	#define DU_X float4( 0.227 , 0.073 , 0.742 , 0.108 ) //Pos I1 = XY Color & I2 = ZW Black 
    #define DU_Y float4( 0.783 , 0.572 , 0.227 , 0.073 ) //Pos I3 = XY Color & J1 = ZW Color
    #define DU_Z float4( 0.798 , 0.108 , 0.783 , 0.572 ) //Pos J2 = XY Black & J3 = ZW Color
	#define DU_W float4( 28.0, 28.0, 28.0, 28.0) //Tresh Hold for Color I & J and Color
	//Inventory Create & Primitive Workbench
	#define DV_X float4( 0.140 , 0.168 , 0.1595, 0.815 ) //Pos K1 = XY Color & K2 = ZW Black 
    #define DV_Y float4( 0.870 , 0.861 , 0.245 , 0.128 ) //Pos K3 = XY Color & L1 = ZW Color
    #define DV_Z float4( 0.7485, 0.149, 0.753 , 0.872 ) //Pos L2 = XY Black & L3 = ZW Color
	#define DV_W float4( 29.0, 29.0, 29.0, 29.0) //Tresh Hold for Color K & L and Color

	//Repair Work Bench & Feed Menu
	#define DX_X float4( 0.156 , 0.145 , 0.843 , 0.145 ) //Pos M1 = XY Color & M2 = ZW Black 
    #define DX_Y float4( 0.843 , 0.855 , 0.379 , 0.124 ) //Pos M3 = XY Color & N1 = ZW Color
    #define DX_Z float4( 0.413 , 0.258 , 0.379 , 0.857 ) //Pos N2 = XY Black & N3 = ZW Color
	#define DX_W float4( 30.0, 30.0, 29.0, 29.0) //Tresh Hold for Color M & N and Color
	//Roaming Seller & ???
	#define DY_X float4( 0.152 , 0.125 , 0.158 , 0.143 ) //Pos O1 = XY Color & O2 = ZW Black 
    #define DY_Y float4( 0.847 , 0.125 , 0.000 , 0.000 ) //Pos O3 = XY Color & P1 = ZW Color
    #define DY_Z float4( 0.000 , 0.000 , 0.000 , 0.000 ) //Pos P2 = XY Black & P3 = ZW Color
	#define DY_W float4( 29.0, 29.0, 1000.0, 1000.0) //Tresh Hold for Color O & P and Color	
	...
	#define DA_X 0.025          // ZPD
	#define DF_Y 0.00           // Seperation
	#define DA_Y 45.0          // Near Plane Adjustment
    //#define DA_Z -0.125       // Linerzation Offset
    #define DS_Y 5            // Linerzation Offset Effects only distance if true
	#define DB_Z 0.0375         // Auto Depth Protection
	#define DE_X 3            // ZPD Boundary 
	...
	#define OIL 1           // Set How many Levels We use for RE_Fix 0 | 1 | 2 | 3 if 1 then it's float2(0,0) for OIF and DI_W
    #define OIF float2(0.5,0.375)// Fix enables if Value is > 0.0 
	#define DI_W float2(0.5,2.5) // Like Shift Boundary DG_W But 0 to inf
	#define WND -1.0         //Weapon Near Pushes depth in and adjust perspective to match.
    //#define DG_Z 0.025        // Min Weapon Hands That are apart of world with Auto and Trim
    //#define DS_X float3(0.025,0,1)// Min Weapon bit only triggers when a OIL Level is set and set here on .y
    //#define DE_W 0.50        // Auto
    ...
    //#define DF_W float4(0.0001,0.0,0.0,0.038)// Edge & Scale
  //  #define SDU 1	
    //So hard when it's a lot of menus.
	//Simple Menu Detection
    #define SMD 2 //Off 0 | 1 | 2 | 3 | 4 
    //How to Play A
    #define DW_X float4( 0.843 , 0.252 , 0.843 , 0.474)    //Pos A = XY Any & B = ZW Lock 
    #define DW_Y float2( 0.845 , 0.818 )                   //Pos C = XY 
    #define DW_Z float4( 0.0, 30.0, 0.0, 1000.0)   //Menu Detection Type for A = X, B = Y, & C = Z. The Last Value is a Wild Card amount W is for X and Z. 
    //How to Play B
    #define DT_X float4( 0.843 , 0.252 , 0.843 , 0.683)    //Pos A = XY Any & B = ZW Lock 
    #define DT_Y float2( 0.845 , 0.818 )                   //Pos C = XY 
    #define DW_W float4( 0.0, 30.0, 0.0, 1000.0)   //Menu Detection Type for A = X, B = Y, & C = Z. The Last Value is a Wild Card amount W is for X and Z. 
  ...
  //Multi Menu Detection                    //4 Blocks total and 2 sub blocks per one block.
    ...
    #define MML -4 //Set Multi Menu Leniency   // -4 |-3 |-2 |-1 | Off | 1 | 2 | 3 | 4         //Negitive [0 & 30] No Leniency | Zero [0-1 & 29-30] Some Leniency | Positive [0-2 & 28-30] Most Leniency    
	//Main Menu & Settings
    #define DO_X float4( 0.108 , 0.095 , 0.050 , 0.850 ) //Pos A1 = XY Color & A2 = ZW Black 
    #define DO_Y float4( 0.392 , 0.302 , 0.264 , 0.167 ) //Pos A3 = XY Color & B1 = ZW Color
    #define DO_Z float4( 0.504 , 0.184 , 0.492 , 0.202 ) //Pos B2 = XY Black & B3 = ZW Color
	#define DO_W float4( 30.0, 30.0, 30.0, 30.0) //Tresh Hold for Color A & B and Color
	//Brightness & Audio A
    #define DP_X float4( 0.504 , 0.171 , 0.485 , 0.184 ) //Pos C1 = XY Color & C2 = ZW Black 
    #define DP_Y float4( 0.729 , 0.201 , 0.509 , 0.448 ) //Pos C3 = XY Color & D1 = ZW Color
    #define DP_Z float4( 0.050 , 0.850 , 0.509 , 0.595 ) //Pos D2 = XY Black & D3 = ZW Color
	#define DP_W float4( 30.0, 30.0, 29.0, 29.0) //Tresh Hold for Color C & D and Color	

    //Audio B & Audio C
	#define DQ_X float4( 0.509 , 0.374 , 0.050 , 0.850 ) //Pos C1 = XY Color & C2 = ZW Black 
    #define DQ_Y float4( 0.509 , 0.522 , 0.509 , 0.524 ) //Pos C3 = XY Color & D1 = ZW Color
    #define DQ_Z float4( 0.050 , 0.850 , 0.509 , 0.597 ) //Pos D2 = XY Black & D3 = ZW Color
	#define DQ_W float4( 29.0, 29.0, 29.0, 29.0) //Tresh Hold for Color A1 & A3 and Color
	// Audio D & Audio E
	#define DR_X float4( 0.509 , 0.372 , 0.050 , 0.850 ) //Pos G1 = XY Color & G2 = ZW Black 
    #define DR_Y float4( 0.509 , 0.597 , 0.509 , 0.376 ) //Pos G3 = XY Color & H1 = ZW Color
    #define DR_Z float4( 0.050 , 0.850 , 0.509 , 0.450 ) //Pos H2 = XY Black & H3 = ZW Color
	#define DR_W float4( 29.0, 29.0, 29.0, 29.0) //Tresh Hold for Color G & H and Color 

    //Save 1 & Save 2
	#define DU_X float4( 0.219 , 0.314 , 0.050 , 0.850 ) //Pos I1 = XY Color & I2 = ZW Black 
    #define DU_Y float4( 0.509 , 0.314 , 0.219 , 0.437 ) //Pos I3 = XY Color & J1 = ZW Color
    #define DU_Z float4( 0.050 , 0.850 , 0.509 , 0.437 ) //Pos J2 = XY Black & J3 = ZW Color
	#define DU_W float4( 29.0, 26.0, 29.0, 26.0) //Tresh Hold for Color I & J and Color
	//Save 3 & Save 4
	#define DV_X float4( 0.219 , 0.571 , 0.050 , 0.850 ) //Pos K1 = XY Color & K2 = ZW Black 
    #define DV_Y float4( 0.509 , 0.571 , 0.219 , 0.710 ) //Pos K3 = XY Color & L1 = ZW Color
    #define DV_Z float4( 0.050 , 0.850 , 0.509 , 0.710 ) //Pos L2 = XY Black & L3 = ZW Color
	#define DV_W float4( 29.0, 26.0, 29.0, 26.0) //Tresh Hold for Color K & L and Color

	//Controls Cam & Controls Key
	#define DX_X float4( 0.148 , 0.170 , 0.387 , 0.187 ) //Pos M1 = XY Color & M2 = ZW Black 
    #define DX_Y float4( 0.370 , 0.200 , 0.387 , 0.170 ) //Pos M3 = XY Color & N1 = ZW Color
    #define DX_Z float4( 0.626 , 0.183 , 0.609 , 0.200 ) //Pos N2 = XY Black & N3 = ZW Color
	#define DX_W float4( 30.0, 30.0, 30.0, 30.0) //Tresh Hold for Color M & N and Color
	//Controls Controller & English Options Kind of working
	#define DY_X float4( 0.622 , 0.170 , 0.605 , 0.183 ) //Pos O1 = XY Color & O2 = ZW Black 
    #define DY_Y float4( 0.848 , 0.196 , 0.142 , 0.258 ) //Pos O3 = XY Color & P1 = ZW Color
    #define DY_Z float4( 0.050 , 0.850 , 0.987 , 0.604 ) //Pos P2 = XY Black & P3 = ZW Color
	#define DY_W float4( 30.0, 30.0, 30.0, 0.0) //Tresh Hold for Color O & P and Color	
    ...
    #define NDW 1
	#define DAA 1
#elif (App == 0xCE95FAE )	//Trine 5
    //#define DS_Z 2            // Set View Mode
	#define DA_W 1            // Set Linerzation
    //#define DB_X 1            // Flip
	#define DA_X 0.025      // ZPD
	#define DF_Y 0.000         // Seperation
	#define DA_Y 40         // Near Plane Adjustment
    //#define DA_Z -0.5      // Linerzation Offset
    //#define DS_Y 1            // Linerzation Offset Effects only distance if true
	#define DB_Z 0.05         // Auto Depth Protection
	#define DE_X 1            // ZPD Boundary 
	...
	#define OIL 3           // Set How many Levels We use for RE_Fix 0 | 1 | 2 | 3 if 1 then it's float2(0,0) for OIF and DI_W
    #define OIF float4(0.6,0.5,0.375,0.25)// Fix enables if Value is > 0.0 
	#define DI_W float4(0.25,0.5,1.75,5.0) // Like Shift Boundary DG_W But 0 to inf
	//#define FTM 0             // Fast Trigger Mode If this enabled then Level 1 and > switches instantly.
    ///#define DG_Z 0.500        // Min Weapon Hands That are apart of world with Auto and Trim
    //#define DS_X float2(0.0025,2)// Min Weapon bit only triggers when a OIL Level is set and set here on .y
    //#define DE_W 0.250        // Auto
    ...
    #define MDD 3 //Set Menu Detection & Direction        //Off | 1 Left | 2 Right | 3 Bottom | 4 Top 
	#define COD 1 //Enable or Disable Cut-Off Direction   //Off | 1 Top & Bottom for One & Two or Left and Right for Three & Four 
    #define DN_X float4( 0.254 , 0.040 , 0.835 , 0.978)   //Pos A = XY Any & B = ZW Lock 
    #define DN_Y float4( 0.571 , 0.047 ,  0.0, 0.0)       //Pos C = XY Any & D = ZW Match
    #define DN_Z float4( 0.0, 0.0,  0.0, 0.0)             //Pos E = XY Match & F = ZW Match
	#define DN_W float4( 0.091, 0.0 , 0.0, 0.0 )            //Size = Menu [ABC] D E F
    #define DJ_Y float4( 30.0, 1.0, 30.0, 1000.0)        //Menu Detection Type for A = X, B = Y, & C = Z. The Last Value is a Wild Card amount W is for X and Z. 
    #define DJ_Z float4( 1000., 1000., 1000, 0.1)         //Set Match Tresh and IU Y Elevation or Cut-Off -1-1
    ...
    #define SSA 1 //Off 0 | 1 Square | 2 Circle
    //#define SNA 1 //Off 0 | 1 Trigger on Aim
    #define DDD_X float4( 0.376 , 0.264 , 0.503 , 0.844)  //Pos A = XY Any & B = ZW Lock 
    #define DDD_Y float4( 0.467 , 0.742 , 0.5 , 0.474)      //Pos C = XY Any & UI Postion 
    #define DDD_Z float4( 29.0, 1.0, 28.0, 1000.0) //Detection Value 0-30 for A = X, B = Y, & C = Z. The Last Value is a Wild Card amount W is for X and Z. 
    #define DDD_W float4( 0.361, 0.226, 1.0, 0.1)             //X is Hoz Size and Y is Vert Size, Z is Inversion amount and W is Depth set elevation with 0.0-0.5 > is Auto.

    #define SSB 1 //Off 0 | 1 Square | 2 Circle
    //#define SNB 1 //Off 0 | 1 Trigger on Aim
    #define DEE_X float4( 0.360 , 0.288 , 0.372 , 0.766)  //Pos A = XY Any & B = ZW Lock 
    #define DEE_Y float4( 0.399 , 0.236 , 0.488 , 0.495)      //Pos C = XY Any & UI Postion 
    #define DEE_Z float4( 29.0, 8.0, 29.0, 1000.0) //Detection Value 0-30 for A = X, B = Y, & C = Z. The Last Value is a Wild Card amount W is for X and Z. 
    #define DEE_W float4( 0.360, 0.215, 1.0, 0.1)             //X is Hoz Size and Y is Vert Size, Z is Inversion amount and W is Depth set elevation with 0.0-0.5 > is Auto.
    
	//Simple Menu Detection
    #define SMD 4 //Off 0 | 1 | 2 | 3 | 4 
    //Main Menu
    #define DW_X float4( 0.025 , 0.225 , 0.001 , 0.855)    //Pos A = XY Any & B = ZW Lock 
    #define DW_Y float2( 0.436 , 0.279 )                   //Pos C = XY 
    #define DW_Z float4( 30.0, 8.0, 30.0, 1000.0)   //Menu Detection Type for A = X, B = Y, & C = Z. The Last Value is a Wild Card amount W is for X and Z. 
    //Options / Local Multiplayer
    #define DT_X float4( 0.081, 0.772 , 0.808 , 0.842)    //Pos A = XY Any & B = ZW Lock 
    #define DT_Y float2( 0.859 , 0.772 )                   //Pos C = XY 
    #define DW_W float4( 30.0, 12.0, 30.0, 1000.0)   //Menu Detection Type for A = X, B = Y, & C = Z. The Last Value is a Wild Card amount W is for X and Z. 
	//Save Slots
    #define DAA_X float4( 0.061 , 0.641 , 0.331 , 0.099)    //Pos A = XY Any & B = ZW Lock 
    #define DAA_Y float2( 0.938 , 0.786 )                   //Pos C = XY 
    #define DAA_Z float4( 30.0, 9.0, 30.0, 1000.0)   //Menu Detection Type for A = X, B = Y, & C = Z. The Last Value is a Wild Card amount W is for X and Z. 
	//Map
    #define DBB_X float4( 0.264 , 0.918 , 0.500 , 0.911)    //Pos A = XY Any & B = ZW Lock 
    #define DBB_Y float2( 0.826 , 0.918 )                   //Pos C = XY 
    #define DBB_Z float4( 30.0, 0.0, 30.0, 1000.0)   //Menu Detection Type for A = X, B = Y, & C = Z. The Last Value is a Wild Card amount W is for X and Z. 
    ...
    #define NDW 1
	#define DAA 1
#elif (App == 0x50D85E5C ) //Gone Golfing
    //#define DS_Z 2            // Set View Mode
	#define DA_W 1            // Set Linerzation
    //#define DB_X 1            // Flip
	#define DA_X 0.025         // ZPD
	...
	#define OIL 2           // Set How many Levels We use for RE_Fix 0 | 1 | 2 | 3 if 1 then it's float2(0,0) for OIF and DI_W
    #define OIF float3(0.5,0.375,0.25)// Fix enables if Value is > 0.0 
	#define DI_W float3(0.5,2.5,3.75) // Like Shift Boundary DG_W But 0 to inf
	//#define OIL 2           // Set How many Levels We use for RE_Fix 0 | 1 | 2 | 3 if 1 then it's float2(0,0) for OIF and DI_W
    //#define OIF 0.75 // Fix enables if Value is > 0.0 
	//#define DI_W 1.0 // Like Shift Boundary DG_W But 0 to inf
	//#define FTM 0             // Fast Trigger Mode If this enabled then Level 1 and > switches instantly.
	...
	#define PEW 1
	#define FOV 1
#elif (App == 0xC604BB7C ) //No one Lives under the lighthouse
    //#define DS_Z 2            // Set View Mode
	#define DA_W 1            // Set Linerzation
    #define DB_X 1            // Flip
	#define DA_X 0.030         // ZPD
	...
	#define PEW 1
	#define FOV 1
#elif (App == 0x9F42F43A ) //The Pony Factory
    //#define DS_Z 2            // Set View Mode
	#define DA_W 1            // Set Linerzation
    #define DB_X 1            // Flip
	#define DA_X 0.025         // ZPD
	//#define DF_Y 0.005         // Seperation
	#define DA_Y 15.5         // Near Plane Adjustment
    //#define DA_Z -0.50 //0.0004      // Linerzation Offset
    #define DS_Y 2            // Linerzation Offset Effects only distance if true
	#define DB_Z 0.03         // Auto Depth Protection
	//#define DE_X 2            // ZPD Boundary 
	...
	//#define DI_W 1.0 // Like Shift Boundary DG_W But 0 to inf
	//#define FTM 0             // Fast Trigger Mode If this enabled then Level 1 and > switches instantly.
	#define DG_Z 0.005        // Min Weapon Hands That are apart of world with Auto and Trim
    //#define DS_X float3(0.025,0,1)// Min Weapon bit only triggers when a OIL Level is set and set here on .y
    //#define DE_W 0.25        // Auto
    #define DI_Z 0.040        // Trim
	...
	#define PEW 1
	#define FOV 1
#elif (App == 0x4B40594D ) //Hello Neighbor
    //#define DS_Z 2            // Set View Mode
	#define DA_W 1            // Set Linerzation
    //#define DB_X 1            // Flip
	#define DA_X 0.025         // ZPD
	...
	//#define FTM 0             // Fast Trigger Mode If this enabled then Level 1 and > switches instantly.
	#define DG_Z 0.0275        // Min Weapon Hands That are apart of world with Auto and Trim
    //#define DS_X float3(0.025,0,1)// Min Weapon bit only triggers when a OIL Level is set and set here on .y
    #define DE_W 0.5        // Auto
    #define DI_Z 0.0275        // Trim
    #define DF_W float4(0.0001,0.0015,0.15,0.005)// Edge & Scale
	...
	//#define DS_W 1.5	      // Weapon Depth Limit Location 2
	//#define WFB 1.0                // ZPD Weapon Elevaton for 1 and 2 scales from [0 - 1]
	#define WND 1.25         //Weapon Near Pushes depth in and adjust perspective to match.
	...
	#define PEW 1
	#define FOV 1
#elif (App == 0x7AD5A79E ) //Secret Neighbor
    //#define DS_Z 2            // Set View Mode
	#define DA_W 1            // Set Linerzation
    #define DB_X 1            // Flip
	#define DA_X 0.025         // ZPD
	//#define DF_Y 0.005         // Seperation
	#define DA_Y 137.5         // Near Plane Adjustment
    //#define DA_Z -0.125      // Linerzation Offset
    //#define DS_Y 3            // Linerzation Offset Effects only distance if true
	#define DB_Z 0.05         // Auto Depth Protection
	//#define DE_X 2            // ZPD Boundary 
    ...
    #define DE_W 0.25        // Auto
    #define DI_Z 0.05        // Trim
    #define DF_W float4(0.0001,0.00125,0.025,0.025)// Edge & Scale
	...
	//Lock Out Menu Detectors  
	#define LMD 1 //Off 0 | 1
    #define DCC_X float4( 0.448 , 0.960 , 0.453 , 0.963)  //Pos A = XY Any & B = ZW Lock 
    #define DCC_Y float2( 0.550 , 0.960 )                 //Pos C = XY 
    #define DCC_Z float4( 3.0, 30.0, 3.0, 1000.0) //Menu Detection Type for A = X, B = Y, & C = Z. The Last Value is a Wild Card amount W is for X and Z. 

	//Simple Menu Detection
    #define SMD 4 //Off 0 | 1 | 2 | 3 | 4 
    #define DW_X float4( 0.063 , 0.109 , 0.916 , 0.400)    //Pos A = XY Any & B = ZW Lock 
    #define DW_Y float2( 0.943 , 0.055 )                   //Pos C = XY 
    #define DW_Z float4( 30.0, 3.0, 30.0, 0.0)   //Menu Detection Type for A = X, B = Y, & C = Z. The Last Value is a Wild Card amount W is for X and Z. 
    
    #define DT_X float4( 0.063 , 0.109 , 0.100 , 0.075)    //Pos A = XY Any & B = ZW Lock 
    #define DT_Y float2( 0.896 , 0.058 )                   //Pos C = XY 
    #define DW_W float4( 30.0, 6.0, 30.0, 1000.0)   //Menu Detection Type for A = X, B = Y, & C = Z. The Last Value is a Wild Card amount W is for X and Z. 

    #define DAA_X float4( 0.080 , 0.879 , 0.248 , 0.053)    //Pos A = XY Any & B = ZW Lock 
    #define DAA_Y float2( 0.516 , 0.806 )                   //Pos C = XY 
    #define DAA_Z float4( 30.0, 0.0, 27.0, 1000.0)   //Menu Detection Type for A = X, B = Y, & C = Z. The Last Value is a Wild Card amount W is for X and Z. 

    #define DBB_X float4( 0.063 , 0.109 , 0.916 , 0.526)    //Pos A = XY Any & B = ZW Lock 
    #define DBB_Y float2( 0.943 , 0.055 )                   //Pos C = XY 
    #define DBB_Z float4( 30.0, 3.0, 30.0, 0.0)   //Menu Detection Type for A = X, B = Y, & C = Z. The Last Value is a Wild Card amount W is for X and Z. 

	//Multi Menu Detection                    //4 Blocks total and 2 sub blocks per one block.
    #define MMD 3 //Set Multi Menu Detection  // Off  0 | 1 | 2 | 3 | 4
    #define MMS 2 //Set Multi Menu Selection  // Off  0 | 1 | 2 | 3 | 4                       //0-2 check to 28-30 check Depnding on the option MML
    #define MML -2 //Set Multi Menu Leniency   // -4 |-3 |-2 |-1 | Off | 1 | 2 | 3 | 4         //Negitive [0 & 30] No Leniency | Zero [0-1 & 29-30] Some Leniency | Positive [0-2 & 28-30] Most Leniency    
	//Game
    #define DO_X float4( 0.120 , 0.600 , 0.866 , 0.210 ) //Pos A1 = XY Color & A2 = ZW Black 
    #define DO_Y float4( 0.120 , 0.120 , 0.120 , 0.600 ) //Pos A3 = XY Color & B1 = ZW Color
    #define DO_Z float4( 0.866 , 0.210 , 0.120 , 0.120  ) //Pos B2 = XY Black & B3 = ZW Color
	#define DO_W float4( 16.0, 16.0, 0.0, 0.0) //Tresh Hold for Color A & B and Color
	//Audio & Audio
    #define DP_X float4( 0.120 , 0.600 , 0.865 , 0.396 ) //Pos C1 = XY Color & C2 = ZW Black 
    #define DP_Y float4( 0.120 , 0.120 , 0.120 , 0.600 ) //Pos C3 = XY Color & D1 = ZW Color
    #define DP_Z float4( 0.865 , 0.396 , 0.120 , 0.120 ) //Pos D2 = XY Black & D3 = ZW Color
	#define DP_W float4( 16.0, 16.0, 0.0, 0.0) //Tresh Hold for Color C & D and Color
    //Controls & Controls
	#define DQ_X float4( 0.120 , 0.600 , 0.865 , 0.272 ) //Pos C1 = XY Color & C2 = ZW Black 
    #define DQ_Y float4( 0.120 , 0.120 , 0.120 , 0.600 ) //Pos C3 = XY Color & D1 = ZW Color
    #define DQ_Z float4( 0.865 , 0.272 , 0.120 , 0.120 ) //Pos D2 = XY Black & D3 = ZW Color
	#define DQ_W float4( 16.0, 16.0, 0.0, 0.0) //Tresh Hold for Color A1 & A3 and Color
	//Video 
	#define DR_X float4( 0.120 , 0.600 , 0.857 , 0.148 ) //Pos G1 = XY Color & G2 = ZW Black 
    #define DR_Y float4( 0.120 , 0.120 , 0.120 , 0.600 ) //Pos G3 = XY Color & H1 = ZW Color
    #define DR_Z float4( 0.857 , 0.148 , 0.120 , 0.120 ) //Pos H2 = XY Black & H3 = ZW Color
	#define DR_W float4( 0.0, 16.0, 0.0, 0.0) //Tresh Hold for Color G & H and Color 
    //Block Three
	#define DU_X float4( 0.244 , 0.879 , 0.248 , 0.053 ) //Pos I1 = XY Color & I2 = ZW Black 
    #define DU_Y float4( 0.152 , 0.799 , 0.000 , 0.000 ) //Pos I3 = XY Color & J1 = ZW Color
    #define DU_Z float4( 0.000 , 0.000 , 0.000 , 0.000 ) //Pos J2 = XY Black & J3 = ZW Color
	#define DU_W float4( 30.0, 27.0, 1000.0, 1000.0) //Tresh Hold for Color I & J and Color
	...
	#define PEW 1
	#define FOV 1
#elif (App == 0x857E0017 ) //Little Orpheus
    //#define DS_Z 2            // Set View Mode
	#define DA_W 1            // Set Linerzation
    #define DB_X 1            // Flip
	#define DA_X 0.0375         // ZPD
	...
	//#define DG_W 0.125        // Shift Boundary Out of screen 0.5 and or In screen -0.5
	#define OIL 3           // Set How many Levels We use for RE_Fix 0 | 1 | 2 | 3 if 1 then it's float2(0,0) for OIF and DI_W
    #define OIF float4(0.5,0.375,0.25,0.0001)// Fix enables if Value is > 0.0 
	#define DI_W float4(0.5,1.,2.,10) // Like Shift Boundary DG_W But 0 to inf

    //#define CWH 1             //ZPD Weapon Hand Consideration For Masking  0 is Off | 1 Is Full | 2 Half Right screen Mask And Not to be used with Weapon Profiles.
    //#define WBA 2.0           //ZPD Weapon Boundary Alt Adjust power for CWH 

	//#define FTM 0             // Fast Trigger Mode If this enabled then Level 1 and > switches instantly.
	#define DG_Z 0.335        // Min Weapon Hands That are apart of world with Auto and Trim
    //#define DS_X float3(0.025,0,1)// Min Weapon bit only triggers when a OIL Level is set and set here on .y
    //#define DE_W 0.25        // Auto
    //#define DI_Z 0.05        // Trim
    //#define DF_W float4(0.0001,0.00125,0.025,0.025)// Edge & Scale
    ...
    #define SSA 1 //Off 0 | 1 Square | 2 Circle
    //#define SNA 1 //Off 0 | 1 Trigger on Aim
    #define DDD_X float4( 0.069, 0.087 , 0.500 , 0.312)  //Pos A = XY Any & B = ZW Lock 
    #define DDD_Y float4( 0.110 , 0.284 , 0.5 , 0.493)      //Pos C = XY Any & UI Postion 
    #define DDD_Z float4( 15.0, 0.0, 30.0, 22.0) //Detection Value 0-30 for A = X, B = Y, & C = Z. The Last Value is a Wild Card amount W is for X and Z. 
    #define DDD_W float4( 0.000, 0.250, 1.0, 0.01)             //X is Hoz Size and Y is Vert Size, Z is Inversion amount and W is Depth set elevation with 0.0-0.5 > is Auto.

    #define SSB 1 //Off 0 | 1 Square | 2 Circle
    //#define SNB 1 //Off 0 | 1 Trigger on Aim
    #define DEE_X float4( 0.180, 0.901 , 0.500 , 0.312)  //Pos A = XY Any & B = ZW Lock 
    #define DEE_Y float4( 0.500 , 0.683 , 0.5 , 0.5)      //Pos C = XY Any & UI Postion 
    #define DEE_Z float4( 15.0, 0.0, 0.0, 22.0) //Detection Value 0-30 for A = X, B = Y, & C = Z. The Last Value is a Wild Card amount W is for X and Z. 
    #define DEE_W float4( 0.000, 0.276, 1.0, 0.01)             //X is Hoz Size and Y is Vert Size, Z is Inversion amount and W is Depth set elevation with 0.0-0.5 > is Auto.


	//Simple Menu Detection
    #define SMD 1 //Off 0 | 1 | 2 | 3 | 4 
    #define DW_X float4( 0.385, 0.500 , 0.573 , 0.936)    //Pos A = XY Any & B = ZW Lock 
    #define DW_Y float2( 0.703 , 0.388 )                   //Pos C = XY 
    ...
    #define DT_X float4( 0.069, 0.087 , 0.500 , 0.317)    //Pos A = XY Any & B = ZW Lock 
    #define DT_Y float2( 0.110 , 0.284 )                   //Pos C = XY 
    #define DW_W float4( 15.0, 0.0, 30.0, 1000.0)   //Menu Detection Type for A = X, B = Y, & C = Z. The Last Value is a Wild Card amount W is for X and Z. 
	...
	#define BMT 1             // ZPD and World Scale Balance // I need to phase this out.
	#define DF_Z 0.1625        // Set the Balance  
    #define DL_Y 0.5        // De-Artifact Only works on some View Modes and causes performance degredation
	//#define DB_Y 1.0          // Effects De-Artifacts -1 to 1 Most of the time leave this at 0 and if you set 1 it takes depth into account 
    #define DL_Z -0.75         // Compat Power
	//#define DJ_X 0.02        // Range Smoothing
...
/*  
	//Lock Out Menu Detectors  
	#define LMD 0 //Off 0 | 1
    #define DCC_X float4( 0.8835, 0.956 , 0.982 , 0.954)  //Pos A = XY Any & B = ZW Lock 
    #define DCC_Y float2( 0.500 , 0.004 )                 //Pos C = XY 
    #define DCC_Z float4( 1000.0, 1000.0, 1000.0, 1000.0) //Menu Detection Type for A = X, B = Y, & C = Z. The Last Value is a Wild Card amount W is for X and Z. 
*/
/*
    #define MDD 1 //Set Menu Detection & Direction        //Off | 1 Left | 2 Right | 3 Bottom | 4 Top 
	#define COD 0 //Enable or Disable Cut-Off Direction   //Off | 1 Top & Bottom for One & Two or Left and Right for Three & Four 
    ...
    #define DN_Z float4( 0.0, 0.0,  0.0, 0.0)             //Pos E = XY Match & F = ZW Match
	#define DN_W float4( 1.0, 0.0 , 0.0, 0.0 )            //Size = Menu [ABC] D E F
    #define DJ_Y float4( 28.0, 28.0, 20.0, 1000.0)        //Menu Detection Type for A = X, B = Y, & C = Z. The Last Value is a Wild Card amount W is for X and Z. 
    #define DJ_Z float4( 1000., 1000., 1000, 0.0)         //Set Match Tresh and IU Y Elevation or Cut-Off -1-1
*/
/*  
	//Simple Menu Detection
    #define SMD 0 //Off 0 | 1 | 2 | 3 | 4 
...
/*
	//Text Menu Detection
    #define TMD 0 //Off 0 | 1Normal | 2 Inverse | 3 Mirror
    #define DZ_X float4( 0.500 , 0.500 , 0.500 , 0.500)  //Pos A = XY Any & B = ZW Lock 
    #define DZ_Y float2( 0.500 , 0.500 )                 //Pos C = XY 
    #define DZ_Z float4( 1000.0, 1000.0, 1000.0, 1000.0) //Text Detection Type for A = X, B = Y, & C = Z. The Last Value is a Wild Card amount W is for X and Z. 
    #define DZ_W float4( 0.0, 0.0, 0.0, 0.0)             //Text Distance from screen, Text Cutoff point, and adjustable XY Masking.
*/
/*  //Multi Menu Detection                    //4 Blocks total and 2 sub blocks per one block.
    #define MMD 1 //Set Multi Menu Detection  // Off  0 | 1 | 2 | 3 | 4
    #define MMS 0 //Set Multi Menu Selection  // Off  0 | 1 | 2 | 3 | 4                       //0-2 check to 28-30 check Depnding on the option MML
...
/* Miscellaneous Depth Buffer Corrections
	...
	#define IWS 0    //IWS Is to Isolate a part of ZPD Boundary Detection, so the Cut-Off point does not mask it out of detection.
    #define DCC_W 0.875 
...
// X = [Position A & B] Y = [Position C] Z = [A ABCW Menu Tresholds] W = [B ABCW Menu Tresholds]
#ifndef DW_X
...
// X = [Text Position A & B] Y = [Text Position C] Z = [ABC Menu Tresholds] W = [Text Adjustment]
#ifndef DZ_X
    #define DZ_X TPos_XY_XY_A_B_D
#endif
#ifndef DZ_Y
    #define DZ_Y TSPos_XY_C_D
#endif
#ifndef DZ_Z
    #define DZ_Z TMenu_Tresh_n_WC_D
#endif 
#ifndef DZ_W
	#define DZ_W Text_Lift_Cutoff_XYDirection_D
#endif

// X = [Position A & B] Y = [Position C] Z = [C ABCW Menu Tresholds] W = [Null W]
#ifndef DAA_X
    #define DAA_X C_SPos_XY_XY_A_B_D
#endif
#ifndef DAA_Y
    #define DAA_Y C_SPos_XY_C_D
#endif
#ifndef DAA_Z
    #define DAA_Z C_Menu_Tresh_n_WC_D
#endif 
#ifndef DAA_W
	#define DAA_W A_Null_W
#endif

// X = [Position A & B] Y = [Position C] Z = [D ABCW Menu Tresholds] W = [Null W]
#ifndef DBB_X
    #define DBB_X D_SPos_XY_XY_A_B_D
#endif
#ifndef DBB_Y
    #define DBB_Y D_SPos_XY_C_D
#endif
#ifndef DBB_Z
    #define DBB_Z D_Menu_Tresh_n_WC_D
#endif 
#ifndef DBB_W
	#define DBB_W B_Null_W
#endif

// X = [Lock Position A & B] Y = [Lock Position C] Z = [Lock ABCW Menu Tresholds] W = [Isolating Weapon Stencil Amount]
#ifndef DCC_X
    #define DCC_X Lock_Pos_XY_XY_A_B_D
#endif
#ifndef DCC_Y
    #define DCC_Y Lock_SPos_XY_C_D
#endif
#ifndef DCC_Z
    #define DCC_Z Lock_Menu_Tresh_n_WC_D
#endif 
#ifndef DCC_W
	#define DCC_W Isolating_Weapon_Stencil_Amount_D
#endif

// X = [Position A & B] Y = [Position C & UI Pos] Z = [ABCW Stencil Menu Tresholds] W = [Stencil Adjust]
#ifndef DDD_X
    #define DDD_X UI_A_SPos_XY_XY_A_B_D
#endif
#ifndef DDD_Y
    #define DDD_Y UI_A_SPos_XY_C_n_XY_Pos_D
#endif
#ifndef DDD_Z
    #define DDD_Z UI_A_Menu_Tresh_n_WC_D
#endif 
#ifndef DDD_W
	#define DDD_W UI_A_Stencil_Adjust_D
#endif

// X = [Position A & B] Y = [Position C & UI Pos] Z = [ABCW Stencil Menu Tresholds] W = [Stencil Adjust]
#ifndef DEE_X
    #define DEE_X UI_B_SPos_XY_XY_A_B_D
#endif
#ifndef DEE_Y
    #define DEE_Y UI_B_SPos_XY_C_n_XY_Pos_D
#endif
#ifndef DEE_Z
    #define DEE_Z UI_B_Menu_Tresh_n_WC_D
#endif 
#ifndef DEE_W
	#define DEE_W UI_B_Stencil_Adjust_D
#endif
...
#endif 
#ifndef COD
    #define COD Menu_Cutoff_D                  //Menu Cutoff Option
#endif 
#ifndef SDU
...
#endif 
#ifndef  AWZ
    #define AWZ Anti_Weapon_Hand_ZFighting_D   //Anti-Weapon Hand Z-Fighting
#endif 
#ifndef  FRM
    #define FRM SM_Foveated_Mode_D             //Enable or disable Foviated Rendering
#endif 
#ifndef  TMD
    #define TMD Text_Menu_Detection_D          //Text Menu Detection & Mirroing Options
#endif 
#ifndef  LMD
    #define LMD Lock_Out_Menu_Detection_D      //Lock Out Menu Detection
#endif 
#ifndef  WBS
    #define WBS Weapon_Boundary_Switch_D       //Weapon Boundary Switch
#endif 
#ifndef  LHA
    #define LHA Lower_Height_Adjust_D          //Lower Height Adjustment
#endif 
#ifndef  SUI
    #define SUI Stencil_UI_Detection_D         //Stencil UI & Detection
#endif 
#ifndef  SNA
    #define SNA Stencil_A_Trigger_D            //Stencil Trigger on Aim Alpha
#endif 
#ifndef  SSA
    #define SSA Stencil_Shape_Alpha_D          //Stencil Shape Alpha
#endif 
#ifndef  SNB
    #define SNB Stencil_B_Trigger_D            //Stencil Trigger on Aim Beta
#endif 
#ifndef  SSB
    #define SSB Stencil_Shape_Beta_D           //Stencil Shape Beta
#endif
#ifndef  IWS
    #define IWS Isolating_Weapon_Stencil_D     //Isolating Weapon Stencil
#endif
#ifndef  ASA
    #define ASA Auto_Scaler_Adjust_D           //Auto Depth Scaling Bool
#endif
...
{   if (WP == 2)
    ...
    if (WP == 20)//float4(0.278,90.0,9.0,0.050);
        Weapon_Adjust = float4(0.278,200.0,9.05,0.050);   //WP 18 | Half-Life 2
    if (WP == 21)
    ...
    if (WP == 27)
        Weapon_Adjust = float4(0.403,5.875,0.0,0.0);      //WP 25 | AMID EVIL RTX
...
{   if (WP == 2)
    ...
    if (WP == 19)
        Weapon_Adjust = float4(1.550,100.0,0.150,0.0);    //WP 17 | Halo Infinite //float4(1.550,117.5,0.125,0.125);
...
{   if (WP == 2)
    ...
    if (WP == 7)
        Weapon_Adjust = float4(0.4,6.0,0.996,0.0);        //WP 5  | Avatar Frontiers of Pandora
    if (WP == 8)
        Weapon_Adjust = float4(1.250,25.0,0.99,0.125);    //WP 6  | Thief
    if (WP == 9)
        Weapon_Adjust = float4(4.6,60.0,0.0,0.0);         //WP 7  | Warhammer 40K Boltgun
    if (WP == 10) //17?
        Weapon_Adjust = float4(0.283,17.500,0.880,0.0);   //WP 8  | Tiny Tina's Wonderland
    if (WP == 11)
        Weapon_Adjust = float4(0.25,0.0,0.0,0.0);         //WP 9  | Diablo IV
    if (WP == 12)
        Weapon_Adjust = float4(3.750,17.5,0.0,0.0);       //WP 10 | THE MULLER-POWELL PRINCIPLE
    if (WP == 13)
    ...
    if (WP == 21) //3.0.x
        Weapon_Adjust = float4(0.8,72.5,7.0,0.0);         //WP 19 | Chasm The Rift
    if (WP == 22)
        Weapon_Adjust = float4(0.265,30.0,0.990,0.0);     //WP 20 | Starfield
    if (WP == 23)
        Weapon_Adjust = float4(6.0,25.0,0.0,0.0);         //WP 21 | RodoQuest
    if (WP == 24)
        Weapon_Adjust = float4(0.251,5.0,0.99935,0.0);    //WP 22 | The Invincible
    if (WP == 25)
...
{   if (WP == 2)
    ...
    if (WP == 7)
        Weapon_Adjust = float4(0.850,95.00,0.815,0.025);  //WP 5  | Wolfenstein: The New Order #C770832 / The Old Blood #3E42619F
...
float4 Weapon_Profiles(float WP ,float4 Weapon_Adjust)
{   if (WP == 2)
        Weapon_Adjust = float4(0.283,150.0,0.88205,0.0);  //WP 0  | The Land of Pain
    if (WP == 3)
        Weapon_Adjust = float4(22.0,50.0,0.001,0.50);     //WP 1  | Turok 
    if (WP == 4)
        Weapon_Adjust = float4(12.5,25.0,0.0,0.0);        //WP 2  | Turok 2
    if (WP == 5)
        Weapon_Adjust = float4(0.5,60.0,2.375,0.0);       //WP 3  | Turok 3: Shadow of Oblivion
    if (WP == 6)
        Weapon_Adjust = float4(2.288,25.0,0.0,0.500);     //WP 4  | Narcosis
    if (WP == 7)//was 2.5 but the fan made me increase it.
        Weapon_Adjust = float4(2.9,35.0,0.0,0.0);         //WP 5  | Talos Principle 2
    if (WP == 8)
        Weapon_Adjust = float4(10.0,50.0,0.0,0.0);        //WP 6  | GhostRunner 2
    if (WP == 9)
        Weapon_Adjust = float4(0.3,81.25,9.0,0.0);        //WP 7  | Zortch
    if (WP == 10)
