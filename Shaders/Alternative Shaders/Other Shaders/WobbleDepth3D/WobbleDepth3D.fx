		...
		if (Wobble_Mode == 0)
			{
				...
				if(Custom_Sidebars == 0)
					{
					color = tex2D(SamplerCLMIRROR,float2(texcoord.x + Perspective * pix.x,texcoord.y));
					}
					else if(Custom_Sidebars == 1)
					{
					color = tex2D(SamplerCLBORDER,float2(texcoord.x + Perspective * pix.x,texcoord.y));
					}
					else
					{
					color = tex2D(SamplerCLCLAMP,float2(texcoord.x + Perspective * pix.x,texcoord.y));
					}	
				}
				else if(w > 0.75)
				{
				if(Custom_Sidebars == 0)
					{
					color = tex2D(SamplerCRMIRROR,float2(texcoord.x - Perspective * pix.x,texcoord.y));
					}
					else if(Custom_Sidebars == 1)
					{
					color = tex2D(SamplerCRBORDER,float2(texcoord.x - Perspective * pix.x,texcoord.y));
					}
					else
					{
					color = tex2D(SamplerCRCLAMP,float2(texcoord.x - Perspective * pix.x,texcoord.y));
					}	
				}
				else
				{
