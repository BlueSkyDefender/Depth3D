...
uniform int Disocclusion_Selection <
	ui_type = "combo";
	ui_items = "Off\0Normal\0Normal & Depth Based\0";
...
if(AO == 1)
	...
	A += 5.5f; // Normal
	B = DBD * 11.0f; // Depth
	dirA = float2(0.5f,0.0f);
	dirB = float2(0.5f,0.0f);
...
void Encode(in float4 position : SV_Position, in float2 texcoord : TEXCOORD0, out float4 color : SV_Target0) //zBuffer Color Channel Encode
{
	...
	float DepthR = 1.0f, DepthL = 1.0f,MSL = (Divergence * 0.25f) * pix.x, S, MS = Divergence  * pix.x;
...
{
	float2 Znum, TCL, TCR, TexCoords = texcoord;
	...
	else if (Stereoscopic_Mode == 3)
	{
		...
		{		
			#if Convergence_Extended
			//L Near
			[flatten] if(Decode(float2(TCL.x-i*pix.x,TCL.y)).y < (1-TCL.x)+pix.x * 0.2 )
						B_DepthL = i*pix.x;
			
			//R Near
			[flatten] if(Decode(float2(TCR.x+i*pix.x,TCR.y)).x < TCR.x+pix.x * 0.2 )
						B_DepthR = i*pix.x;
			#endif
			//L
			[flatten] if(Decode(float2(TCL.x+i*pix.x,TCL.y)).y > (1-TCL.x)+pix.x * 0.2 )
