...
{	
	const float samples[4] = {0.25, 0.50, 0.66, 1.0};
	...
	{	
		...
		{	
			DepthL =  min(DepthL,DepthMapOne(float2((texcoord.x*2 + P)+uv.x, texcoord.y)).r);
			DepthR =  min(DepthR,DepthMapTwo(float2((texcoord.x*2-1 - P)-uv.x, texcoord.y)).r);
		}
		else if(Stereoscopic_Mode == 1)
		{
			DepthL =  min(DepthL,DepthMapOne(float2((texcoord.x + P)+uv.x, texcoord.y*2)).r);
			DepthR =  min(DepthR,DepthMapTwo(float2((texcoord.x - P)-uv.x, texcoord.y*2-1)).r);
		}
		else
		{
			DepthL =  min(DepthL,DepthMapOne(float2((texcoord.x + P)+uv.x, texcoord.y)).r);
			DepthR =  min(DepthR,DepthMapTwo(float2((texcoord.x - P)-uv.x, texcoord.y)).r);
		}
	}
