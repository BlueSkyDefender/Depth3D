...
texture texDML  { Width = BUFFER_WIDTH/Depth_Map_Division; Height = BUFFER_HEIGHT/Depth_Map_Division; Format = RGBA32F;}; 
...
texture texDMR  { Width = BUFFER_WIDTH/Depth_Map_Division; Height = BUFFER_HEIGHT/Depth_Map_Division; Format = RGBA32F;}; 
...
void  DisOcclusion(in float4 position : SV_Position, in float2 texcoord : TEXCOORD0, out float4 colorR : SV_Target0, out float4 colorL : SV_Target1)
{
 float DP;
 ...
 int Con;
	...
	{
	colOutOne = tex2D(SamplerDML,texcoord);
	colOutTwo = tex2D(SamplerDMR,texcoord);
	}
...
{			
			...
			pass DepthMapToTex
		{
			...
			RenderTarget0 = texDML;
			RenderTarget1 = texDMR;
		}
			pass DisOcclusion
		{
			VertexShader = PostProcessVS;
