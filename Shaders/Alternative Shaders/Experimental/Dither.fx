...
uniform int Dither_Mode <
	ui_type = "combo";
	ui_items = "Off\0Dither Mode B/W Noise\0Dither Mode RGB Noise\0Dither Mode B/W Noise Temp\0Dither Mode RGB Noise Temp\0";
...
float4 Dither(float2 texcoord : TEXCOORD0)
{	
	...
	float noise_A = frac(sin(dot(texcoord * A, float2(12.9898, 78.233))) * 43758.5453 * A);
	float noise_B = frac(sin(dot(texcoord * B, float2(12.9898, 78.233))) * 43758.5453 * B);
	float noise_C = frac(sin(dot(texcoord * C, float2(12.9898, 78.233))) * 43758.5453 * C);
...
float4 Out(float4 position : SV_Position, float2 texcoord : TEXCOORD) : SV_Target
{
	...
	float4 Color = Dither(texcoord),Done,Website,D,E,P,T,H,Three,DD,Dot,I,N,F,O;
