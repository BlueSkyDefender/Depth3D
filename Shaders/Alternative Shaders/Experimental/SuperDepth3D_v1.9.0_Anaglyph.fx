...
uniform int Anaglyph_Colors <
	ui_type = "combo";
	ui_items = "Red/Cyan\0Dubois Red/Cyan\0Dubois Green/Magenta\0Magenta/Green\0";
...
void PS0(float4 position : SV_Position, float2 texcoord : TEXCOORD0, out float4 color : SV_Target)
{
	...
	if(Polynomial_Barrel_Distortion == 0)
	{
		...
		{	
				...
				if(Perspective_Edge_Selection == 0)
				{
				C = tex2D(SamplerCLBORDER,float2(texcoord.x + Perspective * pix.x,texcoord.y));
				CT = tex2D(SamplerCRBORDER,float2(texcoord.x - Perspective * pix.x,texcoord.y));
				}
				else
				{
				C = tex2D(SamplerCLMIRROR,float2(texcoord.x + Perspective * pix.x,texcoord.y));
				CT = tex2D(SamplerCRMIRROR,float2(texcoord.x - Perspective * pix.x,texcoord.y));
				}
			...
			if (Anaglyph_Colors == 0)
			{
				...
				if(Perspective_Edge_Selection == 0)
				{
				color =  (tex2D(SamplerCLBORDER,float2(texcoord.x + Perspective * pix.x,texcoord.y))*LeftEyecolor) + (tex2D(SamplerCRBORDER,float2(texcoord.x - Perspective * pix.x,texcoord.y))*RightEyecolor);
				}
				else
				{
				color =  (tex2D(SamplerCLMIRROR,float2(texcoord.x + Perspective * pix.x,texcoord.y))*LeftEyecolor) + (tex2D(SamplerCRMIRROR,float2(texcoord.x - Perspective * pix.x,texcoord.y))*RightEyecolor);
				}
			}
			else if (Anaglyph_Colors == 1)
			{
				...
				float3x3 R = float3x3(-0.011,-0.032,-0.007,
									  ...
									  float3 NL = normalize(mul(N,L));
									  float3 NR = normalize(mul(N,R));


				color = float4(C.rgb * NL + CT.rgb * NR, C.a);
			}
