...
/////////////////////////////////////////L/R/DepthMap Pos//////////////////////////////////////////////////////////
	...
	float DWAL = +WA;
	float DWAR = +WA;
	color.r =  texcoord.x-Depth*pix.x*SbSdepthR(float2(texcoord.x-DWAR*pix.x,texcoord.y));
	color.gb =  texcoord.x+Depth*pix.x*SbSdepthL(float2(texcoord.x+DWAL*pix.x,texcoord.y));
	}
	else
	{
	float DWAL = 1+WA;
...
void PS_renderL(in float4 position : SV_Position, in float2 texcoord : TEXCOORD0, out float3 color : SV_Target)
{
		...
		float D = Depth/2;
		color.rgb = tex2D(ReShade::BackBuffer, float2(texcoord.x-D*pix.x, texcoord.y)).rgb;
