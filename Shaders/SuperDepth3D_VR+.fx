	...
	///**SuperDepth3D_VR+**///
	//--------------------////
	#define SD3DVR "SuperDepth3D_VR+ v4.0.4\n"
	//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
...
namespace SuperDepth3DVR
{
	#define D_ViewMode 1
		...
		// DAA_X = [Position A & B] DAA_Y = [Position C] DAA_Z = [ABCD Menu Tresholds] DAA_W = [Null]
		...
		//Triggers
		static const float IWS = 0, SUI = 0, SSA = 0, SNA = 0, SSB = 0, SNB = 0, FRM = 1, LHA = 0, WBS = 0, TMD = 0, AWZ = 0, CWH = 0, WBA = 0, WFB = 0, WND = 0, WRP = 0, MML = 0, SMD = 0, WHM = 0, SDU = 0, ABE = 2, LBE = 0, DRS = 0, MAC = 0, ARW = 0, OIL = 0, MMS = 0, NVK = 0, NDG = 0, FTM = 0, SPO = 0, MMD = 0, SMP = 0, LBR = 0, HQT = 0, AFD = 0, MDD = 0, FPS = 1, SMS = 1, OIF = 0, NCW = 0, RHW = 0, NPW = 0, SPF = 0, BDF = 0, HMT = 0, HMC = 0, DFW = 0, NFM = 0, DSW = 0, LBC = 0, LBS = 0, LBM = 0, DAA = 0, NDW = 0, PEW = 0, WPW = 0, FOV = 0, EDW = 0, SDT = 0;
		//Overwatch.fxh State
		#define OSW 1
	#endif
...
uniform int Extra_Information <
	...
	sampler SamplerzBufferVR_L
		...
		#else
		texture texzBufferBlurVR < pooled = true; > { Width = BUFFER_WIDTH / 8.0 ; Height = BUFFER_HEIGHT / 8.0; Format = R16F; MipLevels = 3; }; // Needs to be RG16F If external Texture is given for DownSample. Not needed if external texture is already down sampled.
		#endif	
	...
	{   //Luminance
		return saturate(tex2Dlod(SamplerLumVR,float4(texcoord ,0,11)).y);//Average Luminance Texture Sample
	}
	...
	*/
	...
	{
		int Num_of_Values = 9;
		float2 Shift_XY = floor(texcoord * Res * pix * Num_of_Values);	
	...
	float2 Auto_Balance_Selection()
	{
		...
		float4 XYArray[9] = { float4 ( 0.0  , 0.0, 0.0  , 0.0),        						//Off                  0
							  ...
							  float4 (0.0,0.0,0.0,0.0),                                        //Eye Tracker          7
	...
	float4 DepthMap(in float4 position : SV_Position,in float2 texcoord : TEXCOORD) : SV_Target
	{
		...
		float3 Fade_Pass_A = float3( tex2D(SamplerzBuffer_BlurVR,float2(0,0.083)).x,
									 ...
									 tex2D(SamplerzBuffer_BlurVR,float2(0,0.416)).x );
		float3 Fade_Pass_B = float3( tex2D(SamplerzBuffer_BlurVR,float2(0,0.583)).x,
									 tex2D(SamplerzBuffer_BlurVR,float2(0,0.750)).x,
									 tex2D(SamplerzBuffer_BlurVR,float2(0,0.916)).x );
	...
	{	float WConverge = 0.030, D = MD_WHD.x, Z = Zero_Parallax_Distance, WZP = 0.5, ZP = 0.5, W_Convergence = WConverge, WZPDB, Distance_From_Bottom = lerp(0.9375,1.0,saturate(WFB)), ZPD_Boundary = ZPD_Boundary_n_Fade.x, Store_WC;
	...
	{   
		...
		#else
		Blur_Out.xy = StoredTC < 0.5 ? Text_Mask : Invert_Depth_Mask;
		#endif
	}
	
	#if SUI
		#if SSA == 1//Square
		float Stencil_Masking(float2 TC, float2 Pos, float2 UI_Mask_Size, float UI_Mask_Inversion)
		{
			TC += Pos - 0.5;
	...
	static const float  VMW_Array[9] = { 0.0, 1.0, 2.0, 3.0 , 3.5 , 4.0, 4.5 , 5.0, 5.5 };		
	float GetDB(float2 texcoord)
	{
		float LR_Depth_Mask = 1-saturate(tex2Dlod(SamplerzBuffer_BlurVR, float4( texcoord  * float2(0.5,1) + float2(0.5,0), 0, 3.0 ) ).x * 2.5);
		...
		uint VM_Mip_Cal = VMW_Array[clamp(View_Mode_Warping,0,8)];
		float VMW = View_Mode == 5 ? lerp(0,VM_Mip_Cal,Base_Depth_Buffers.x) : VM_Mip_Cal;
		/*
		...
		*/			
		...
		if( DZ_W.x > 0 && Text_Menu_Detection())
		{
			...
			if(Stencil_n_Detection_A())
				...
				//Auto Depth 0.5 > needs more detection points will update that later.
				float UI_B_Mask_Depth = DEE_W.w < 0.5 ? DEE_W.w : saturate(tex2Dlod(SamplerzBufferVR_L, float4( 1-UI_B_Mask_Pos, 0, 4) ).x + (0.5-DDD_W.w));
				float2 UI_B_Mask_Size= DEE_W.xy;
				if(Stencil_n_Detection_B())
					DepthBuffer_LP.xy = lerp(DepthBuffer_LP.xy,UI_B_Mask_Depth,Stencil_Masking(texcoord,UI_B_Mask_Pos,UI_B_Mask_Size,DEE_W.z));
				#endif
		#endif	
	...
	float GetMixed(float2 texcoord)
	{
		...
		return tex2Dlod(SamplerzBufferVR_Mixed,float4(Shift_TC,0,0)).x;
	}
	...
	{   
		...
		float LR_Depth_Mask = saturate(tex2Dlod(SamplerzBuffer_BlurVR, float4( Coordinates  * float2(0.5,1) + float2(0.5,0), 0, 3.0 ) ).x * 2.5);
		float GetDepth = smoothstep(0,1, tex2Dlod(SamplerzBufferVR_L, float4(Coordinates,0, 4.0) ).x), CB_Done = fmod(CBxy.x+CBxy.y,2),
		...
		float MinNum = 20, MaxNum = MaxMix, D = abs(Diverge), Cal_Steps = D * Perf,
				   ...
				   "             Do this once you set your 3D settings of course."; >
	{
			...
			pass Blur_DepthBuffer
		{
			...
			RenderTarget0 = texzBufferBlurVR;
		}
		#endif
	#else
			pass Blur_DepthBuffer
