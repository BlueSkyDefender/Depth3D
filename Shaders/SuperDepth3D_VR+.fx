...
namespace SuperDepth3DVR
{
		...
		//Triggers
		static const int ARW = 0, OIL = 0, MMS = 0, NVK = 0, NDG = 0, FTM = 0, SPO = 0, MMD = 0, SMP = 0, LBR = 0, HQT = 0, AFD = 0, MDD = 0, FPS = 1, SMS = 1, OIF = 0, NCW = 0, RHW = 0, NPW = 0, IDF = 0, SPF = 0, BDF = 0, HMT = 0, HMC = 0, DFW = 0, NFM = 0, DSW = 0, LBC = 0, LBS = 0, LBM = 0, DAA = 0, NDW = 0, PEW = 0, WPW = 0, FOV = 0, EDW = 0, SDT = 0;
		//Overwatch.fxh State
		#define OSW 1
	#endif
	...
	*/
	...
	{   //Check Depth
		...
		float Trigger_Fade_A = Detect, Trigger_Fade_B = Detect_Out_of_Range >= 1 ? 1 : 0, Trigger_Fade_C = Detect_Out_of_Range >= 2 ? 1 : 0, Trigger_Fade_D = Detect_Out_of_Range >= 3 ? 1 : 0, AA = Auto_Adjust_Cal(ZPD_Boundary_n_Fade.y), 
			  PStoredfade_A = tex2D(SamplerLumVR,float2(0, 0.250)).z, PStoredfade_B = tex2D(SamplerLumVR,float2(0, 0.416)).z, PStoredfade_C = tex2D(SamplerLumVR,float2(1, 0.416)).z, PStoredfade_D = tex2D(SamplerLumVR,float2(1, 0.250)).z;
		//Fade in toggle.
		return float2x4( float4(PStoredfade_A + (Trigger_Fade_A - PStoredfade_A) * (1.0 - exp(-frametime/AA)), //exp2 would be even slower 
						 ...
						 float4(saturate(Detect_Out_of_Range * 0.25),0,0,0) );
	}
