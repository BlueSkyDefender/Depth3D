...
//depth information
	...
	float4 depthR = ReShade::GetLinearizedDepth(float2((texcoord.x*2-1), texcoord.y));
		...
		//Assassin Creed Unity | Call of Duty: Ghost | Call of Duty: Black Ops 2 | Call of Duty: Advance Warfare
		if (AltDepthMap == 7)
		{
...
{	
	...
	for (int j = 0; j <= x; j++) 
	{
		if (tex2D(SamplerCC, float2((texcoord.x > 0.5)-j*pix.x,texcoord.y)).r && tex2D(SamplerCC, float2(texcoord.x+j*pix.x,texcoord.y)).r <= texcoord.x+pix.x) 
		{
...
{	
	...
	for (int i = 0; i <= x; i++) 
	{
		if (tex2D(SamplerCC, float2((texcoord.x < 0.5)+i*pix.x,texcoord.y)).r && tex2D(SamplerCC, float2(texcoord.x+i*pix.x,texcoord.y)).r <= texcoord.x-pix.x)
		{
			if (Pop >= 1)
			{
